...
- **🔒 …and the guard-rail's first version was itself bypassable (caught by review).** It read the live mutable `openNotes` flag at endGame, but `buildNotes` bakes the mechanic into per-note properties once (`n.open`, `n.hopo`) and hit-detection keys on *those*, not the flag. So `?open=1` → mid-run `__rrOpenNotes(false)` → flag false, chart still easy → **ranked**. `_chartShapeModified()` now **asks the chart, not the flag**: `notes.some(n => n.open || n.hopo || n.type==='rail')`. That is immune to flag toggles and also covers MULTIPLAYER, where `notes` is the host's injected chart (its broadcast serializer carries `open`/`hopo`), so a guest inherits the host's verdict instead of consulting local flags. Proven with a clean A/B on the same track+difficulty (365 notes both): chart with `opens:2` + flag flipped off mid-run → **unranked** (`practice`, `preview:true`, never reaches `session.submit`); chart with `opens:0` → **ranked**. STARSTRUCK stays ranked (array-identical).
- **🔒 SPOTLIGHT ×2 XP was inflating the retention ledger (found pre-review).** The bonus granted double XP by calling `goals.js recordLevelComplete()` twice — a function that also increments the daily levels counter and re-runs `claimDaily`/`claimWeekly`. Proven with a positive control on identical seeds: ONE real spotlight clear took a player from 1 level → **3**, claimed the daily goal a play early, and **advanced their streak 4 → 5 without earning it**. Streak is the retention ledger and gates the freeze economy. Fixed by adding an XP-only primitive `RhythmGoals.grantBonusXp(amt, label)` (touches `rr_xp` alone; no counters, no claims, no streak) and routing the bonus through it. XP still doubles exactly; ledger untouched.
- **⚡ SPOTLIGHT perf regression (confirmed by review).** `isSpotlight()` → `spotlightTrack()` → `dailyPicks()` filtered and `O(N log N)`-sorted the entire catalog (**1258 tracks**, two `hashStr` per comparison) on **every rendered song row, shelf card and coverflow cover** — 100+ full catalog sorts to fill the All-Songs list. Memoized on the day-key + catalog length (the pick still rotates at midnight and still resolves once the catalog loads). Measured after: **2000 `isSpotlight()` calls in 1.1 ms** (0.55 µs each), identity-stable.
- **C3 Weekly-Rift ladder (jukebox.js):** the hub card gains your rung + Δ since yesterday + a closing clock + top-3, off the now-live `GET /rift/standings`. Local day-snapshot delta (sign-verified). Fail-soft: error/401/anon/`my_rank:null` → the sub-block is *removed*, leaving index.html's card exactly as it paints today.
- **B4 freeze ceremony (jukebox.js):** covers the library-return path index.html's hub reveal misses, sharing goals.js's one-shot `consumeFreezeJustUsed()` so the two surfaces can never double-fire. **B3 SPOTLIGHT chips** on covers/rows/shelf cards. `applyAttentionCap()` extended — the rift climb-glow is the lowest-priority accent and gets `.rr-hush` under any higher pulse.
- **A3 calibration surfacing (game.js):** the pause overlay now shows the live median timing offset ("Your hits are landing ~40ms late", ≥12-sample floor, outlier-robust median) with a RE-CALIBRATE shortcut into the existing build104 wizard; the auto-cal chip names the consequence ("Applied +20ms — notes will feel later"); a consent-gated `calibrate_apply` telemetry event fires at both apply sites. Injected from game.js (index.html untouched), de-duped across repeated pauses.
- **`?playtest=1` master switch (game.js; owner-requested).** One URL flag ORs on every dark mechanic (rails + STARSTRUCK + open notes) for a single page load — **never writes localStorage**, so it cannot leak into a later session — with an unmistakable "PLAYTEST BUILD — dark flags ON · UNRANKED" corner chip. Playtest runs are unranked by construction.
- **D4 typography ladder + D2 mobile (index.html + jukebox.css):** a 5-size token ladder at `:root` with **66** fractional `font-size` one-offs snapped to rungs (each ≤0.5px, tight controls verified unaffected); ≥44px tap targets on 14 measured controls at 375px (grown hit-areas, not glyphs), safe-area insets, and `scrollWidth <= clientWidth` asserted at both 375px and 768px.
- **D3 is PARTIAL, and marked so:** `.rr-skel` shimmer + `.rr-shelf-empty` shipped as CSS with a documented contract, but the JS that applies them lives in jukebox.js which that agent didn't own. The wiring is folded into build165's hub work (same file owner) rather than claimed as done.
- **Hard-difficulty question settled by killing it (analysis only, no code).** A production query showed Hard's "6.1% max-combo / 72.7% completion" is not a population: **20 of 22 plays and all 16 completions belong to ONE player**; two others tried once and never returned. Fable's `HARD_RETUNE_BRIEF_v1.md` (revised after its own stale-cohort hypothesis was refuted by a provenance query) resolves on its pre-committed rule: **FRACTURE BRIDGE only — no lever moves, no CHART_VERSION 3.** The lever table stands as a spec in reserve.
node --check clean on all six touched files (goals.js joins the set). `?v` bumped BEFORE the boot smoke (the `rr-verify` server has no no-cache headers, so an unchanged `?v` can serve stale JS) and `game.js?v=463` confirmed loaded from the DOM.

### build163 — Wave 4b: the reclaim loop closes + encore chain + STARSTRUCK (anchored, dark)  ✅ node-clean · boot-verified (0 console errors) · adversarial-reviewed (1 confirmed → fixed) · CHART_VERSION 2 safety proven · owner-playtest-gated (feel)  ·  ?v=462
Three disjoint agents (catalog.js / game.js / index.html+jukebox.js+css) executing Fable's NIGHT_ROADMAP_v2, plus a mid-wave Fable ruling (`SCORING_RULINGS_v2.md`) that redirected A2 away from a leaderboard-breaking design. Adversarial review found **1 real defect** (below) — fixed before this commit. `multiplayer.js` was deliberately frozen this wave so the owner's pending 2-account MP test stays bisectable.
- **B1 — the "PASSED YOU" reclaim loop closes (catalog.js + jukebox.js).** `getReclaimTargets()` is a SYNC, never-null, cached getter backed by the now-live `GET /leaderboard/deltas?since=` (maps the server's snake_case to a fixed camelCase contract; sanitizes + `scrubName`s the passer; drops rows where `passerScore <= myScore` or the track can't resolve to a playable non-video track; caps at 10). Signed-out is the common beta case and is **completely silent** — no token, no network call — with split guards (`_reclaimInflight` / `_reclaimDone`) so a later sign-in still fetches exactly once. It dispatches `rr:reclaim` only when ≥1 row lands. The welcome card gains a TOP-PRIORITY branch — "**CrimsonHalo** passed you on *Molten Run* · RECLAIM IT" — whose click calls `launchTrack(track, {targetScore, targetName})`, i.e. **straight into build162's ghost chase**. Positive framing always ("reclaim", never "you lost"); one per session; silent on `[]`. This is the composition Fable staked the return-rate on: a specific human takes your spot, one tap puts you in a race against their score.
- **B2 — the encore is no longer a lie (game.js + catalog.js).** It was a renamed PLAY AGAIN offering *the same song* at the peak-end moment. Now an S/A/FC finish arms `RhythmCatalog.encoreNextTrack()` — a curated next track (same genre → BPM ±10% only when both carry a finite `bpm` → unplayed-first → decode-confident, stable-hash tiebreak, never the current track, never a video) — and accepting increments a **session-only** ENCORE ×n chain shown on the next results. A chain of 3 stamps a SETLIST badge into its own `rr_setlist` key (never `rr_career`). Absent getter or `null` → byte-identical same-song replay, no dead button.
- **A2 — STARSTRUCK, redefined by Fable, and it now actually fires (game.js; DARK behind `rr_odphrase`).** As first built it was a *correct mechanism attached to a chart that could never trigger it*: the phrase rule was GH's "≥3 stars within 4s", but `placeStars` emits ~1 star per 18s, so real charts produced `phrases = 0`. The implementing agent refused to "fix" that by moving stars and escalated — correctly. **Fable's ruling (`SCORING_RULINGS_v2.md`): don't move a single star; redefine the phrase.** Root cause of the trap: `placeStars` can convert an *accent* into a star, and stars don't carry accent pay (`accPerf` gates on `type === 'accent'`) — so re-placing stars changes which notes forfeit their 750-point PERFECT bonus, which literally moves score-ceiling terms and forces CHART_VERSION 3, segmenting the live beta boards. Instead, a **STARSTRUCK phrase is now the approach run that ENDS at each existing star**: `_tagStarPhrases(arr, diff)` walks backward from each star collecting eligible preceding notes (plain tap/accent only; no holds/rails/chords/bombs/fillers), star last-in-time, non-overlapping, `PHRASE_MIN=3`, `PHRASE_WIN` {easy 6 / medium 5 / hard·rift 4}s, `PHRASE_CAP` {4/6/8/8}. Members get a gold rim in `drawNote` ("an invisible phrase is a slot machine"). OD charge stays exactly per-note; the ceremony IS the reward.
- **REVIEW FINDING (confirmed, fixed): hopo-resolved phrase members never credited.** `chainHopos()` resolves flow notes with `n.judged = true; n.hit = 'great'` **without** the normal hit path — so `_phraseCredit` never ran for them, and because the note was now `judged` the miss sweep skipped it too, meaning `_phraseVoid` never ran either. Any phrase whose approach run got flow-chained was left permanently **un-completable and un-voided** — a clean full carry into the star silently failed to fire the ceremony. (Hopos are gated by `rr_open`, so with both dark flags off there was no production impact — but it would have bitten the instant either flipped.) Fixed with one line: `_phraseCredit(n)` in `chainHopos`, a no-op for untagged notes so flag-OFF stays byte-identical.
- **D1/C2 — hub attention cap + RIFT board (index.html + jukebox.js/css).** A real governor, `applyAttentionCap()`, enforces **one pulsing accent at a time** with precedence `reclaim > streak-at-risk > everything else` (verified: with a reclaim card present the at-risk chip's `animationName` collapses to `none`). The leaderboard gains the **RIFT** segment (the client already submitted `difficulty='rift'`; the trophy was invisible), an honest "mixed tiers" sub-label on All, a last-played default from `rr_diff` instead of All, and a branded RIFT empty state. **Partial:** the `#menu-hub` tile grid was NOT restructured — the agent judged it already satisfies "one hero, quiet shelves" and declined a high-risk, non-headless-verifiable teardown. Fable's D1 is therefore only partly done; the hub-grid composition remains open.
- **CHART_VERSION-2 safety, proven non-vacuously.** `phraseV2Proof` rebuilds `buildNotes` on FIXED beats (so `analyzeBeats` isn't re-run; `buildNotes` contains no `Math.random`) with the flag off, then on. On a real 151s chart at every difficulty: notes off === on (213/298/365/510), `arraysIdentical: true`, `firstDiffIndex: -1`, `phrases 0 → 7`, lengths exactly `PHRASE_CAP`. The only delta is the added `phraseId`/`phraseLen` props. No star moves → no accent forfeits its bonus → **no ceiling term moves → rides at CHART_VERSION 2, scores stay submittable.** `score +=` statement set unchanged (17 occurrences); `curMult`/`JUDGE`/`MAX_MULT`/`placeStars`/the 0.14 star-charge line all untouched.
- **Verification method fixed.** `rr-verify` serves via plain `python -m http.server` (no no-cache headers, unlike `serve.py`), so a reload during a build cycle can serve **stale cached JS** keyed by an unchanged `?v`. From this build on, `?v` is bumped BEFORE the boot smoke; the DOM was checked to confirm `game.js?v=462` actually loaded. An earlier proof in this session was caught being vacuously true for the same class of reason (an "unchanged" localStorage assertion against a profile that had no keys) — both now carry positive controls.
Backend note: Lovable **deployed** the edge function this wave, so `/leaderboard/deltas`, `/rift/standings`, `room-announce`/`room-resolve`, `in_match`, and `/plays today_percentile` are all LIVE — the Wave-2/3 features are no longer dark. Edge-function source does NOT auto-deploy on a chat turn; it needs an explicit deploy step, separate from the game-file sync. **Owner-playtest-gated:** the STARSTRUCK flag flip (`rr_odphrase`), the encore chain's feel, the reclaim card's tone (the friend-or-slot-machine sniff test), and the bridge-run copy from build162.

### build162 — Wave 4a: GHOST CHASE from the leaderboard + FIRST PULSE bridge run  ✅ node-clean · boot-verified (0 console errors) · unranked law empirically proven · owner-playtest-gated (feel)  ·  ?v=461
Executes the top two bets of Fable's NIGHT_ROADMAP_v2 — "the leaderboard is data when it should be drama" (C1) and the data-confirmed Easy→Medium cliff (A1). Both agents were killed mid-run by an API session limit **at the verification step**, so the edits landed unverified; I finished the verification by hand rather than trusting them (see proofs below). Ceiling-safe: no `score +=` site, `CHART_VERSION`, `MAX_MULT`, `curMult`, or `notes_total*1500` line touched.
- **C1 — GHOST CHASE: "RUN THIS DOWN" from every leaderboard row (jukebox.js + jukebox.css).** The engine already shipped a *display-only* gold ghost pill + "BEAT IT! 🏆" flash (`setGhostTarget`, game.js:6343–6371) and `launchTrack(track,{targetScore,targetName})` already threads to it (catalog.js:1884) — but the ONLY caller was buried in multiplayer.js, so a solo player on an 852-track catalog with live boards had **zero ways to race a human**. This wires the launch side: a self-contained `ghostChase()` IIFE augments the By-Song board's rows (and the chase card) with a RUN THIS DOWN button → `launchTrack` with that row's score+name. Gated: skips your own row (`.you`), placeholder rows, zero/invalid scores, unplayable/video tracks, and any board that can't be resolved to a single track (fail-soft → no button). Track resolution is header title/artist/artwork matched against `allTracks()`, disambiguated by artwork, `null` on ambiguity. A 16ms-coalesced MutationObserver re-augments on re-render. **Zero catalog.js/game.js edits** — the seam already existed.
- **A1 — FIRST PULSE bridge run (game.js + catalog.js).** The cliff is FOUR simultaneous axis jumps (density `DIFF_STEP` 4→2, approach 1.30→0.95, hit window 0.20→0.16, lane-jump 1→2, + chords/bombs) — and production data agrees: Easy *completes lower* than Medium (82% vs 86%). Fix, with zero chart edits and zero ranked surface: from an **Easy** results screen at grade **≥ A**, one inviting card ("READY FOR PULSE? — take the bridge run") launches the same track's **real Medium chart** judged at **Easy's forgiving 0.20 window**, **UNRANKED**. `effHitWindow(diff)` is the single seam — it returns Easy's window ONLY while `_bridgeRun` is armed, else `diff.hitWindow` verbatim, so a normal Easy/Medium run judges byte-identically; chart-BUILD clearances still read `DIFFICULTY[...].hitWindow` so the Medium chart layout is untouched. catalog.js routes the bridge **before** the server-chart branch so it can never reach a scored `liveProvider`. Clearing (grade A/S) → "PULSE-READY" toast + pre-selects Medium; a non-clear or a mid-song quit (`_abortBridge`) restores the Easy tier.
- **Verification done by hand (the killed agents never got here):** `playUrl.length === 3` confirms the `{bridgeRun}` opt actually threads `playUrl → play(prov,opts)`. Every run-start path traced: `startAt()` (MP) calls `play(prov)` with no opts → hits the `else { _bridgeRun=false }` branch, so **a stale bridge can never leak into a multiplayer run**; the practice re-arm is `if(!_practiceOn) return` and can't fire during a bridge; only `restartGame()→beginPlay()` preserves the flag, which is the intended "restart keeps the bridge armed". **UNRANKED law proven empirically, with a positive control:** `recordLocal({…preview:true})` wrote *nothing*, while the identical payload without `preview` wrote `rr_career`/`rr_xp`/`rr_goals`/`rr_bonus_sparks` — so the probe genuinely detects writes and the bridge genuinely blocks them. Boot: all modules load, `bridgeReady()` returns false without throwing, all 9 required DOM ids exist (so `ghostChase`'s defensive retry terminates on the first tick), no stray results injections, **0 console errors**.
**Owner-playtest-gated:** whether the bridge actually *eases* the cliff, and whether "bridge run" reads as invitation rather than remedial, are feel calls for the 60fps machine.

### build161a — fix: include multiplayer.js Wave-3 work omitted from build161  ✅ node-clean  ·  ?v=460
Integration error, caught and corrected. build161 (0e6ffee) staged catalog/game/index/jukebox but **not multiplayer.js**, so the MP join-by-code + `in_match` roster flag its own CHANGELOG entry describes never landed in the commit — nor in the Lovable sync fired from that SHA. This commits exactly that already-reviewed W3-B work and re-syncs. `?v` 459→460 so the corrected multiplayer.js is refetched rather than cache-shadowed. Still HELD for the owner's 2/3-account matrix.

### build161 — Overnight Wave 3: backend-wired retention + MP join-by-code + streak surfacing  ✅ node-clean · boot-verified (0 console errors) · adversarial-reviewed (0 findings) · MP HELD · fail-soft (backend routes not yet deployed)  ·  ?v=459
Autonomous overnight run, wave 3 — wiring the game to the new backend surface Lovable shipped in parallel, plus streak surfacing. Three disjoint-file agents (game.js+catalog.js / multiplayer.js / jukebox.js+css); a 3-dimension adversarial review returned **0 findings**; integrated boot smoke = 0 console errors. **All three features FAIL-SOFT** and were confirmed to degrade to nothing when their backend endpoint is missing — which matters because a live probe found the build160 backend routes are written but **not yet deployed to the production edge function** (`/room-resolve` → 404; `/tracks` 200, `/presence/online` 401, so the function is up but the new routes aren't registered). Flagged to Lovable to deploy; the features light up with zero further client changes once live.
- **Retention — "TOP N% TODAY" social proof (game.js + catalog.js, display-only):** the `/plays` POST response now (server-side) carries `today_count` + `today_percentile`; catalog.js captures them into `_lastPlayStats` + `getLastPlayStats()` and — because the submit is async and resolves AFTER the synchronous results render — dispatches a `window` `rr:playstats` event so game.js's `_renderTodayPct()` paints/repaints a results pill ("TOP 8% TODAY · of 214 today", or a friendlier "N players today · climb the board" past ~60th pct). Reuses the exact `_ratingsTidFor` real-solo gate; renders nothing when stats are null (offline / older server / preview / MP). No `score +=` / ceiling change — pure social proof.
- **MP — join-by-code + "in a match" roster flag (multiplayer.js; HELD):** **in_match** — `/presence/online` now returns a per-user `in_match`; `renderOnline` shows an "IN A MATCH" pill and disables the CHALLENGE button for them (three layers: rendered `disabled`, cooldown-repaint skips them, `battleCall` guard), all `=== true`-strict so an absent field is byte-identical to today. **Join-by-code** — host mints + displays a shareable room code (`_roomAnnounce` → `POST /room-announce`, shown in the host-only `!room.show` `#mpx-invite-code` slot, revoked on `closeRoom` via `{closing:true}`); a "Join by code" lobby input (`_mountJoinByCode`, idempotent, `isTrusted`-guarded) resolves via `_roomResolve` → `GET /room-resolve` → the existing `_pendJoin` funnel. Fully fail-soft (404/network → "code not found or expired", never throws). **Zero new timers** (reuses `_pendJoinT`); softPresence-only, no new channels, spectate byte-identity held.
- **Retention — always-on daily-streak chip (jukebox.js + jukebox.css):** a `#rr-streak-chip` (flame + day count) in the library `.lib-actions` header, the persistent complement to build160's once-per-session welcome card. Reads `RhythmGoals.streakState()` (real shape `{count,lastCompleteDate,…}`, no alive/atRisk field → derived from `count`+`lastCompleteDate` like the welcome card's `_streakAtRisk`): steady gold when alive today, ember-pulse when at-risk (played yesterday, not today), nothing at count 0 / lost / RhythmGoals absent. Reduce-motion collapses the pulse; taps into the career overlay. Empty-guarded, no layout disturbance.
node --check clean on all touched files; boot smoke = 0 console errors. MP HELD for the owner's 2/3-account matrix. **Backend deploy gap flagged to Lovable** (build160 edge-function routes 404 on prod — code in repo, not deployed); Wave-4 backend order also fired (rank-deltas for the "PASSED YOU" reclaim loop, Rift standings, Hard-combo funnel pull).

### build160 — Overnight Wave 2: retention rails + ceiling-safe scoring feel + MP reliability + UI polish  ✅ node-clean · boot-verified (0 console errors) · adversarial-reviewed (0 confirmed / 1 refuted) · MP HELD  ·  ?v=458
Autonomous overnight run, wave 2. Four disjoint-file agents (jukebox.js+css / game.js / multiplayer.js / index.html) built on the Wave-1 data layer + Fable's SCORING_RULINGS_v1, then a 4-dimension adversarial-review workflow (per-file skeptic → adversarial refute) returned **0 confirmed defects** (the one JUKEBOX finding — a near-miss "SO CLOSE" branch unreachable with production data — was refuted as a harmless graceful-degrade dead branch, reachable anyway under `?mock=1`). Integrated build boot-verified headless: node-clean, zero console errors, all six Wave-1 retention getters present + null-safe.
- **Retention — the rails render (jukebox.js + jukebox.css):** the Wave-1 catalog data layer now surfaces as UI. **R-3 mastery crowns** (gold ♔ on cover cards + song rows where best grade is S; pooled-cover-safe class toggle) + **collection meters** (per-genre "♔ 12/40 S", shown only when that genre has ≥1 S). Three shelves injected atop Browse via `renderRetentionShelves`: **FOR YOU** (`forYouTracks`), **FRESH SINCE YOU WERE HERE** (`freshSince`, subtitle from `lastVisitAgeHours`), **DAILY PICKS** (date-stable `dailyPicks`). **R-2 welcome-back card** (`#rr-welcome`, absolute-mounted so it never disturbs the grid) — streak-at-risk branch (from `RhythmGoals.streakState()`) wins over fresh-content branch; one-per-session (`rr_welcome_shown`), dismiss-for-the-day, yields to calibration/intro surfaces. **#17 near-miss micro-tags** ("SO CLOSE" ≥0.97 acc / "1 TO S" grade-A) on rows + shelf cards. Every feature empty-guarded (no data → no shelf/card/chip). Read-only consumer of catalog/goals getters; reused `--rr-*` tokens + `rrFbIn`; gold ♔ the one sanctioned non-crimson accent (GB/green-fret precedent) — no purple/blue/green.
- **Gameplay feel — ceiling-safe scoring layer (game.js; per Fable's SCORING_RULINGS_v1, all cosmetic/FX/derived-read):** **FEVER ladder** — a display-only tier band (WARM→BLAZE→GOLDEN→…→ASCENDANT) driven by `combo`, injected into `#mult-gauge` with a "NEXT in N" bar; the honest `xN` badge is **untouched** (at combo 120 the badge still reads `3x` while FEVER reads "BLAZE · GOLDEN in 30"). **OD legibility** — an "OVERDRIVE +N" end-tally that *diffs* `score − _odScoreAtFire` (zero new `score +=`), a READY density cue, a results OD stat block. **OD overcharge reservoir** — render-only spectacle from the meter charge the byte-identical clamp discards (never read by any scoring path). **Hold ceremony** (HOLD!→CLEAN!→RIDE MASTER by clean-streak) + **SLIP recolor** (`#dad7d2`→`#9a938c` warm smoke). **HARD-LAW PROOF:** all 15 `score +=` sites + 24 load-bearing scoring lines byte-identical; `CHART_VERSION`=2, `MAX_MULT`=4 unchanged; a scripted 600-event replay scored **bit-identical (365458) on both the pristine and edited builds**. Leaderboard-safe.
- **MP — reliability (multiplayer.js; ALL HELD behind the owner's 2-account matrix):** **#22** poll-rail ACCEPT now threads the real `notificationId` → POSTs `/challenge/accept` + `ackNotice` (was silently dropped → re-rang on reload). **#21** (highest-risk) mid-run drop → forfeit guard: end-of-song `final` routed through the MP-5 retry queue (idempotent `onFinal`, match-scope dropped in teardown), the match channel now *rebuilds* on a live-run websocket flap (removed the `matchLive` short-circuits; SUBSCRIBED setup re-asserts stay `!matchLive`-gated so a live rebuild never sprays setup) + a 12s opp-gone grace so the rebuilt roster doesn't false-forfeit. **#23** caller MISSED window 45s→90s (covers the full ring+join budget). **#20** room-start deadlock → host watchdog: a stranded guest re-broadcasts `room-ready` every 2s, host re-emits `room-start` (raw send by design, bounded `<6`, dedupe-by-mid). Invariants held: softPresence-only, public rr-* channels, spectate byte-identity, every new timer (`_oppGoneGraceT`/`_readyRebcT`) cleared in all teardowns.
- **UI — polish (index.html, CSS + `<head>` meta only):** **FOUT fix** — 97 intended-Chakra-Petch stacks → `sans-serif` (the 4 deliberate mono-chain hybrids + JetBrains status chips preserved). **Lane-color unification** — canonical `--lane-0..4` tokens mirroring game.js `LANE_COLORS`, applied to keycaps/set-pads/test-pips (added the missing 5th keycap; three divergent color languages collapsed to one). **Results pulse cap** — kept the one-shot entrance cascade, neutralized the redundant infinite loops (two hero pulses survive). **Label legibility floor** — sub-10px labels floored to 10px, over-video HUD labels lifted off `--ink-dim` with a shadow. **Share/OG** — added `og:image`/`twitter:image` (→ existing `assets/title-wordmark.png`, verified present) + `summary_large_image`.
node --check clean across all touched files; integrated boot smoke = 0 console errors. MP HELD for the owner's 2/3-account matrix before Publish. **Backend caught up in parallel:** Lovable landed the overnight order — MP realtime rails re-verified ALL-GREEN (`notifications` in the realtime publication, `bc:user:*` RLS scoped, `/challenge` `private:true`, accept/decline reach the caller), + 3 new endpoints ready for Wave 3 (`room-announce`/`room-resolve` join-by-code, `/presence/online` `in_match`, `/plays` `today_count`+`today_percentile`). MP funnel views live but empty until an emitting build is Published.

### build159 — Overnight Wave 1: MP hardening + retention + UI design-system (Fable NIGHT_ROADMAP + discovery sweep)  ✅ node-clean · adversarial-reviewed (5 confirmed → all fixed) · MP HELD  ·  ?v=457
Autonomous overnight run, wave 1. Fable's NIGHT_ROADMAP_v1 (31 tasks/5 tracks) + a 23-item discovery sweep drove 4 disjoint-file agents (multiplayer.js / game.js / catalog.js+chat.js / index.html+jukebox.css), then two adversarial-review workflows (catalog+game+ui, and a 3-lens MP pass) confirmed 5 real defects — **all fixed before this commit**.
- **MP — bulletproofing (multiplayer.js; ALL HELD behind the owner's 2-account matrix):** **MP-1 ghost-peer dev harness** `window.__mpBot` (`?mpbot=1`+localhost; a 2nd anon Supabase client with its own storageKey → MP is finally headless-provable over public channels); **MP-2 versioned room-state snapshot beat** (4s + on-mutation, converge-from-highest-version, normal-rooms-only → retires the "missed one broadcast = divergent room" class); **MP-5 critical-send retry queue** (ack-or-retry on channel flap, dedupe, ≤3/10s, flush on SUBSCRIBED); **MP-4 explicit `mpPhase` state machine** + illegal-transition telemetry (observe-only). Review fixes: rematch-retry ordering (was wiped by resetForRematch's `_critQ=[]`), room-scope queue preserved across resub (`!_roomResubActive` gate), idempotent `softPresence.start()` (no heartbeat stacking). Invariants held: no `channel.track()`, public rr-* channels, spectate byte-identity, timers cleared in all 3 teardowns.
- **Retention — results moment (game.js, results-only, real-run-gated):** **R-1 near-goal line** ("N NOTES FROM FULL COMBO" / accuracy-to-next-grade) + wired RETRY, dedups the old `#results-almost`; **G-1 trouble-section → DRILL THIS SECTION** (clusters your misses → one-tap into Practice at 0.75×); **G-3 "vs your best" pace strip** (10-segment gold/ember timeline, game.js-side localStorage). No new score site; HUD decree intact.
- **Retention — data layer (catalog.js; rendering lands Wave 2):** `getAffinity`/`forYouTracks`, `getCollectionStats`/`chaseTheS`, `freshSince`/`lastVisitAgeHours` (+ the one `rr_lastVisit` stamp), `dailyPicks` — all derived + graceful-empty.
- **Profanity filter — real bug fixed (catalog.js + chat.js):** the display-name filter was a substring match that censored legit names ("grape", "Dickinson", "Scunthorpe", "Cockburn"). Rebuilt as **two tiers** — HARD slurs by leet-fold substring (catches `nigger123`/`xnigger`/`n1gg3r` evasions), collision-prone words whole-word + digit-adjacency + allowlist (passes the legit names). Verified **18/18 censored, 26/26 passed**; word list unchanged (union verified). (A first pass over-corrected and opened an evasion hole — caught by the review, then fixed.)
- **UI — design system (index.html CSS + jukebox.css + `tools/brand-linter.py`):** **U-1** `--rr-*` design tokens at `:root` (palette/radii/spacing/motion) with legacy vars re-pointed as byte-identical aliases + a standing **brand linter** (flags cool darks / off-palette hues; whitelists GH-green gems + JetBrains chips; 4 genuine cool scrims warmed → 0 violations); **U-2** one feedback vocabulary (toast / status line / card, lifted out of the MP namespace); **U-4** one motion ladder (140/220/420ms + easing pair), reduce-motion verified. Visually near-identical, now consistent.
node --check clean across all touched files. MP HELD for the owner's 2/3-account matrix before Publish. Playtest-gated (owner machine): everything owner-feel in NIGHT_ROADMAP §"strictly owner-playtest-gated". Wave 2 wires the catalog retention data into rails/cards + the discovery gameplay/UI fixes.
...
- **game.js — Easy rail teaching ladder**: on Easy only, `_railFromHold` returns `'no'` until ≥2 plain holds have charted (hold→hold→rail is the tutor). Medium/Hard/RIFT + flag-OFF byte-identity untouched.
- **game.js — ceremony de-collide**: RIFT-unlock toast 1200→2400ms so grade → ratings (1.2s) → unlock (2.4s) land as three beats.
- **catalog.js/jukebox.js — RIFT trophy surfaced** (bug-class): `getBest`/GRADE-UP now scan `['easy','medium','hard','rift']` and the sheet BEST line names the tier (`BEST S · … · RIFT`) — rift bests were written but never read back (invisible trophy).
- **jukebox.js — cover `onerror` ghost fix**: the thumbnail-fail retry only re-tries the full URL when it differs from the thumb (`_tu !== artwork_url`), else falls straight to the branded tile — a non-Supabase 404 no longer strands a broken-image ghost. Supabase covers byte-identical.
- **multiplayer.js — roster truth-chips**: a departed opponent renders a crimson **LEFT** (keyed on `room._lastP2`), a missed call **NO ANSWER**, instead of the ghost WAITING row. Review fix: `return` before the trailing repaint so the NO ANSWER paint isn't synchronously clobbered back to WAITING.
- **multiplayer.js — waiting-state escalation**: one `#mpx-waitstatus` funnel with live `m:ss` elapsed + copy escalation at 20s/60s; conn-loss precedence preserved; interval cleared in all 3 teardown paths (extends build155 hygiene).
- **multiplayer.js — B1 telemetry funnel**: 10 consent-gated `mp_*` events (call_sent / ring_shown{rail} / answered / join_converged{path,ms} / ready / start / error{stage} / conn{state}) via a guarded `_tel` — so the owner's 2-account test yields DATA, not console archaeology. Observe-only (no transport change).
- **Golden Buzzer thumbnails — verified ALREADY FIXED** (owner report): DB proof 37/37 playable GB winners have valid artwork; the Supabase image-transform endpoint returns proper 260KB square thumbnails (HTTP 200); build146 square-fit + triple fallback stand. Separate gap found + driven to Lovable: the `game-catalog` API flags only ~20 of 37 winners `golden_buzzer:true` (join under-coverage) → fix requested, plus RIFT-leaderboard `difficulty='rift'` acceptance confirmation.
MP changes HELD for the owner's 2/3-account matrix before Publish. Playtest-gated (owner machine): Flow-Shield feel, RIFT, rails (`?rails=1`), note-grammar readability.

### build155 — MP review fixes: 6 confirmed adversarial-review bugs + 2 more  ✅ node-clean · HELD (owner 2-account test)  ·  ?v=453
A 5-lens adversarial expert review of build154 (find→refute) confirmed 6 real realtime-lifecycle bugs; a fix pass closed all + found 2 more. multiplayer.js only, +58; invariants held (no `channel.track()`; public `rr-*` channels unchanged; timers cleared in all teardowns; spectate byte-identity intact).
1. **Room re-subscribe infinite loop** — the room rebuild self-reset its own attempt counter each cycle (via `leaveRoomChannel`→`_clearConnResub`), so the ≤20 cap + backoff never engaged (hammered Supabase every 2s, never surfaced OFFLINE). Fixed with a `_roomResubActive` guard.
2. **Both-ready watchdog self-disabled after the first room match** — `_roomStartMid` was never cleared, so `_armBothReadyWd` early-returned on every rematch → the silent "starting…" hang returned. Fixed: null it in `resetForRematch`/`teardownMatch`.
3. **Re-readied guest couldn't rejoin once the host was live** — the `!matchLive` guard refused to re-send `start`. Fixed: host re-broadcasts the same `_startedAtMs` (idempotent, bounded) so the late guest joins.
4. (found) **Call-lifecycle dedupe was dead** (`_callState` never nulled) → a prior call's late accept/decline hijacked the current call; duplicate accepts flipped a resolved call back to JOINING. Fixed with phase/uid guards + null-on-terminal.
5. (found) **Watchdog not disarmed when an opponent left pre-match** → a misleading "couldn't sync" card instead of "opponent left." Fixed.
6. **UI dimension re-verified clean** — `[hidden]` guards present, peer strings via `textContent`/`esc` (no XSS), handlers bound once.

### build154 — MP hardening: client reliability + legibility (Fable MP_HARDENING_v1)  ✅ node-clean · HELD (owner 2-account test)
Executes Fable 5's senior-design + realtime plan for the "call-in / connect / won't-start / who's-where" cluster (multiplayer.js/catalog.js/index.html; game.js untouched). **Reliability:** A2 caller call-lifecycle (consume accepted/declined + presence-truth fallback), A3 un-brick the poll (5-min cooldown not session-permanent; 10s in-lobby cadence), A4 READY re-emit parity for direct duels, A5 bounded channel re-subscribe + real connection state, A6 both-ready watchdog (T+6 retry, T+14 LOUD fail — never an infinite spinner), A7 stranded-callee missed-call recovery card. **Legibility (the owner's deepest pain):** §4.2 connection chip (LIVE/RECONNECTING/OFFLINE), §4.3 **WHO'S HERE** roster with per-player status chips (spectator-gated), §4.4 reason line + failure cards (declined/missed/both-ready/dead-room — never a bare grey banner), B3 dead "share the code" copy → link-first, B4 ring-card avatar + 45s countdown, B5 `_pendJoin` dead-ends = cards. New `RhythmMP.connState()/roomRoster()/callState()`. **BACKEND (Lovable, done):** the real ring unblock — the `/challenge` broadcast now emits `private: true` (it was `private: false`, mismatched with the client's private subscribe → the ring was **never deliverable**); `battle_call_accepted`/`declined` emitted to the caller; `notifications` confirmed in the realtime publication with the SELECT + `bc:user` receive RLS (scoped to `bc:user:*` — public channels unaffected); all endpoints 401-for-anon. Invariants: no `channel.track()`; softPresence + public channels intact. **HELD** — MP is unprovable headless; ships as ONE release validated by the owner's 2/3-account matrix (MP_HARDENING_v1 §5) before Publish.

### build153 — Mechanics wave complete: ratings show-hook + note-grammar (T6) + teaching (T7) + telemetry (T9)  ✅ verified (game.js)
Finishes Fable v2 §B/§C/§D1 + v1 T9. game.js only, +300/-14, no new score sites.
- **Ratings show-hook** — `endGame` un-hides `#results-rate` ~1.2s after the grade on a REAL run only (suppressed for practice/preview/MP/demo); star/chip clicks call `RhythmCatalog.rateTrack` (merge per field), pre-fill via `ratingFor`, keyboard arrow-rove that never hijacks the results Enter/Esc flow. **This closes the ratings feature end-to-end** (client + UI + backend + hook). Proven headless: real shows + writes `rr_ratings`; practice/MP/demo stay hidden; re-run pre-lights.
- **T6 note-grammar** (silhouette-first readability) — chord chrome-capsule crossbar (dumbbell), hold grip-ring + drain arc + 75%-grace notch, accent spike-ring, bomb desaturation. Visual-only (judging/tap-zones/sizes untouched); renders under fxLite/reduceMotion; lane colors kept.
- **T7 teaching** (Easy-only, non-MP) — first hold/rail isolated ≥1.5s; one-time HOLD/SLIDE▸ head glyph, retires after 2 clean completions (`rr_hints`); never P2. Proven: Easy 2 tags, Medium 0, retire→0.
- **T9 telemetry** — hold banked/slipped/dropped, rail transfer made/missed, Flow-Shield fires, rating submits → the existing consent-gated `RhythmTelemetry.event`.
node --check clean; zero console errors. Playtest-gated (owner machine): T6 greyscale legibility, live mechanic telemetry, T7 rail chip in real play.

### build152 — Slide Rails flagship (T2 charter + T4 ribbon + T3 input)  ✅ verified · DARK behind RAILS_ON
The tester's "slide→slide→button" made native: a **rail = a sustain whose lane MOVES**, derived from the audio centroid glide — the fix for the #1 new-player hold→switch pain. game.js only, +485/-30, entirely behind `RAILS_ON()` (`?rails=1` / `localStorage rr_rails`), default OFF. T2 charter (`centroidPath` sampling → `_railFromHold` with ±1 clamp-don't-reflect steps, per-difficulty min-segment + max-transfers, lane-clearance + slide-then-button post-pass, MINGAP hoist); T4 render (`railX` reduces algebraically to `noteX` at integer lanes → ribbon on the measured strings, bend chevrons); T3 input (migrating hold slot; transfers resolve through the EXISTING GRACE/SLIP/DROP bands — **no new score sites, transfers pay 0, ceiling untouched**; keyboard finger-roll / GH fret-change hammer-on / touch drag; P2 mirrored). **Verified:** flag-OFF `__rrChartStats` byte-identical (easy/medium/rift identical; hard core identical, only the Fable-authorized trill/pattern MINGAP hoist differs); flag-ON `railAudit()` = **0 violations** on all 4 tiers, `railGeom()` max|Δ| = **0px** at 2 sizes, scripted transfers correct with **0 score delta**; node --check clean, zero console errors both flag states; `fretGeom` untouched. Playtest-gated (owner + `?rails=1`): GH/touch feel + the TW/drift/min-len tunables. A song with a real pitch-slide is needed to see rails (the ambient demo has none).

### build151 — Ratings client + UI: rate song + difficulty-feel + browse aggregates (Fable v2 D)  ✅ verified · +QA fix
Owner "rate songs / rate difficulty / show scores when browsing." Held-safe, additive — no scoring/economy path touched. `catalog.js`: `rr_ratings` local-first store; `rateTrack({stars?,felt?})` merge-write → `POST /ratings` (signed-in) with `rr_ratings_queue` offline drain mirroring `submitReport`; `ratingFor` pre-fill; track-sheet line + defensive `rating_avg`/`rating_count`/`felt_median` reads. `jukebox.js`: gold `★4.2` (n≥3) in the song row + cover-card badge. `index.html`: `#results-rate` inline widget (5 gold stars + CHILL→UNFAIR chips) + CSS. **Backend (Lovable):** `rr_track_ratings` table (RLS user-only) + trigger-maintained aggregates on `radio_tracks`, `POST /ratings`, aggregates batched into `/tracks` + `/track/:id` (no N+1), `GET /admin/ratings` with difficulty-outlier + content-problem auto-flags. QA fix: `.results-rate[hidden]` guard so the unwired widget doesn't leak onto every results screen (author `display:flex` beat the UA `[hidden]`).

### build150 — MP reliability: the intermittent "call-in / connect / won't-start" cluster  ✅ node-clean · HELD (needs owner 2-account test before Publish)
Roadmap MP bucket (multiplayer.js only), per [reactive-rhythm-mp-intermittent-diagnosis]. Three fixes for "sometimes connects, certain things won't start":
1. **`bc:user` private battle-call channel** — the #1 "call-in doesn't ring" cause. The private topic needs the authed JWT on the realtime socket AT JOIN, but supabase-js wires the session token lazily (after `_adoptSiteSession`), so the first `subscribe()` often joined before the token landed → `CHANNEL_ERROR` → invites dead all session, and the retry stopped the instant the channel *object* existed (even errored). Now: `supa.realtime.setAuth(JWT)` **before** subscribe; the guard checks the **observed SUBSCRIBED** state (`_bcUserLive`), not object-exists; a bounded re-subscribe fires on `CHANNEL_ERROR/TIMED_OUT/CLOSED`; the boot retry stops only on live. Hardened so the 2.5 s retry can't tear down a mid-join channel (release the in-flight guard in the subscribe callback + a 9 s safety).
2. **room→match ready handoff** (the "won't start" deadlock) — the 350 ms one-shot READY-arm missed the softPresence warmup race under load and never retried. New `_armMatchReadyFromRoom()` re-arms on the opponent-present rising edge; a `_same`-selection guard stops a re-broadcast of the *identical* pick from un-readying a peer (a genuine re-pick still un-readies both — playtest-3 Bug D preserved); the match-ready `ready` re-emits every 1.8 s (cap 15) until the opponent readies.
3. **`start` / `room-start` one-shot broadcasts** → bounded self-cancelling re-emits (start 3×/~3.6 s, room-start 4×/~6 s) so a late-subscribed peer/spectator still catches them; receivers idempotent (`_roomStartMid`, `onStart` dedupe by `atMs`) so a re-emit can't double-open/double-start/double-mount. All new timers cleared in teardown/leave/rematch.
**RLS/softPresence-safe:** no `channel.track()`/native presence added; `setAuth` only upgrades anon→authed for signed-in users (public lobby/room/match + softPresence channels untouched); guests never call it. `node --check` clean. **HELD** — MP can't be proven headless; ship to preview, owner runs the 2-account test (procedure in the tracker), then Publish.

### build149 — Content cleanup: media_type seam + hide dev/test junk + bias discovery off dead tracks  ✅ node-clean · HELD (catalog.js)
Roadmap content bucket (catalog.js only, game-side + **non-destructive** — no DB writes). Owner "clean the dead/broken catalog." Confirmed the `game-catalog /tracks` API DOES return `media_type` (1255 audio / 158 video) + `is_video` (the "no field yet" note was stale); `mediaType()` already prefers it (SWAP-SEAM honored). New `isJunkTrack()` — a **title-shape-only** predicate (`/^test\s*\d*$/i`, `/^e2e\s+test/i`, `/^reactivvibeai\.com$/i`) filtered at the `loadCatalog` chokepoint (live + mock) before the music/video split, so dev-test junk ("test", "test00", "E2E Test Song", the site-URL placeholder) disappears from every rail/browse/search. Never matches `artist_name` (the owner's real songs share the Reactivvibeai artist; "Test Drive"/"Testament" are safe). New `_discoverConfident()` + `confidentFirst` sorter float decodable/non-failed tracks to the head of **Featured/Hot/Surprise** (New keeps chronology) so a bad Surprise-Me pick can't burn a first impression — reorder-only, no rail emptied, AI-Flix video levels untouched. Launch/scoring/economy paths byte-identical.

### build148 — Fable's quick wins: Flow Shield (T1) + multiplier-ramp retune (T5) + RIFT Expert tier (T8)  ✅ verified · SYNCED to preview (?v=452)
First execution slice of Fable 5's mechanics direction ([MECHANICS_DIRECTION_v1.md](MECHANICS_DIRECTION_v1.md)) — the three "immediate loop relief" tasks, all `game.js` (+ a RIFT button in `index.html`), all held-safe (no Lovable). Owner said "start Fable's quick wins."

1. **T1 — FLOW SHIELD.** While Overdrive is LIVE, a missed note no longer breaks your combo — it burns ~25% of the OD meter (≈2s of OD time) instead (`FLOW_SHIELD_COST`, playtest-gated). The note still scores 0 and still counts as a miss for accuracy, and **bombs are never shielded** (they resolve on the bomb path with its own reset). A perfect FC has no misses → the shield never fires → the `notes_total*1500` ceiling is untouched (leaderboard-safe). Feedback = the missed string flashes but a crimson-gold **"◆ SAVED"** callout fires and the combo lives — a dopamine beat, not a mercy. Mirrored in the P2 shadow-scorer (co-op) against P2's 6s OD window. Resets each run with the OD state. Converts OD into a two-use strategic resource (double score vs streak insurance) — solves both top-player depth and newcomer mercy with one lever.
2. **T5 — Multiplier-ramp retune (loop reachability, Fable Law 1).** New `_comboStepFor()` makes the combo-per-tier step **difficulty-aware**: Hard & RIFT ramp at **8** (2× @combo 8, 3× @16) instead of the classic 12, so a dense chart actually reaches its first reward (Hard's chain broke every ~15 notes → players lived at 1×, never seeing HOT@25). Easy/Medium keep the classic 12; the `tight` timing profile is left as-tuned. `comboCap`, `MAX_MULT` and the score ceiling are **all untouched** — a perfect FC still tops out at 4× on every tier, so only how *fast* you ramp changes (leaderboard ceiling identical). Routed through all 6 mult read-sites (curMult / count-mult / HUD gauge ×3 / P2). Verified: `__rrDebug.comboStep()` = {easy:12, medium:12, hard:8, rift:8}.
3. **T8 — RIFT (Expert) tier, NEW.** A 4th difficulty that **IS the pre-build141 brutal Hard** — the #1 "too hard" complaint reframed as opt-in aspirational content. `DIFFICULTY.rift` (approach 0.62 / window 0.11, tighter than FRACTURE) + `DIFF_STEP.rift:1`; `buildNotes` restores every build141-softened lever **for RIFT only** via `_rift ? <brutal> : <existing>` and `_hardLike` (bombs, 4-lane rows, 3-note chords, ±3 leaps, 0.22 min-gap, tight trill/pattern gates). **Safe by construction:** every edit is a new `rift` object key or a rift-only branch, so easy/medium/hard are byte-identical. Gated: locked until you clear FRACTURE (Hard) at **≥85%** (one-way `rr_rift_unlocked` latch set in `endGame`, or admin); the engine always accepts `setDifficulty('rift')` for levels/launch/tests — the gate only controls the UI button. New 4th `.diff-btn` (gold "Rift / Expert ◆") in a 2×2 `#diff-grid`; a locked click nudges with a toast instead of switching. **Verified live:** RIFT demo chart = **824 notes / 195 chords / ±3 lane-leaps / 4-lane bomb rows / peak 7nps** (the exact pre-build141 brutal Hard, matching the build147 "824→580" and "~195 chord-partner" notes); Hard stays byte-identical at 580; button present + locked + gate + unlock flow all confirmed.

`node --check` clean. Verified in `rr-verify` preview (comboStep table, rift-vs-hard `__rrChartStats`, the full lock/unlock/gate flow, 2×2 grid, **zero console errors**). T1's OD-active branch can't be exercised headless (rAF is throttled so Overdrive never engages) — it's node-valid, self-contained, error-free, and pending live OD-play confirmation (owner machine / the Wednesday session). **HELD for the Wednesday batch.** Scoring note for the owner: T1+T5 raise *median* Hard scores (the ceiling is unchanged, so anti-cheat is intact and — per build141 precedent — no `CHART_VERSION` bump); flagged for the Wednesday leaderboard-era review. Playtest-gated tunables: `FLOW_SHIELD_COST`, Hard `comboStep`, the RIFT 85% gate.

### build147 — Playtest-6 game-feel batch: practice fix + readability + first-note runway + Hard softening  ✅ verified · HELD (?v=451, Wednesday batch)
Top-player playtest, `game.js` only (the tactical layer under Fable 5's mechanics direction — [MECHANICS_DIRECTION_v1.md](MECHANICS_DIRECTION_v1.md)). Five items:
1. **P0 — Practice Mode "failed to start"** (the tester's 10s-section bug). Root cause: the panel clamps the section to `track.duration_seconds` **metadata**, which can exceed the real decoded buffer (a truncated/preview `audio_url`); a start past the buffer made `DemoPlayer.play(offset)` clamp the seek to the tail 50 ms → the source ended instantly → `practiceRearm()` → a re-arm STORM that never visibly starts. Fix: clamp `_practiceSection` into the real `songDuration` first (end→min, start≥1s inside, collapse→whole-song). Verified: `[0,10]` repro now plays (9 notes), no storm/throw. Gated on `_practiceOn` → normal runs byte-identical.
2. **Gems too small** (tester): `GEM_K` 1.55→1.8 + tighter emphasis span so the *weakest/smallest* gems get the biggest lift (+19% weak / +14% typical diameter); bake resolution raised (`lw*0.92`/82 → `lw*1.15`/108) so bigger gems stay crisp. Proportional to lane width (mobile-safe); string geometry untouched.
3. **Red notes lost on red levels/corners** (tester): a near-black matte rim + soft dark halo drawn UNDER every gem (alloc-free, always-on, on a clean transform) so a gem's silhouette separates from ANY backdrop regardless of lane color/level theme. Gems keep lane colors (green fret-1 preserved) — a CONTRAST fix, not a recolor.
4. **First notes "already down" at start** (tester): a lead-in runway — sectioned practice PRE-ROLLS the audio a hair before the section (musical count-in) so the first note falls the full highway; normal/whole-song runs drop the notes that would spawn already at the strike line (unplayable). **MP-safe:** skipped for injected/host charts + live MP (runway depends on per-peer `userScroll` → dropping would desync seats). Never empties a short clip; chords/time-sort preserved.
5. **Soften HARD** (beta data — Hard was a combo wall: 739 notes/2×-Medium, 0/22 ≥90%, ~6% max combo). Hard-ONLY levers: MINGAP 0.22→0.30, nps 5→4, lane-jump ±3→±2, chord budget halved + time-floor 2.0→3.0s + partner-cap 3→2 + 3-note-fan i%12→i%30, scattered-bomb gap 11→48, bomb-row 14→20s/4→3 lanes. Demo Hard **824→580 notes (−30%)**, chords 195→86 (−56%); an avg beta Hard (~739) lands ~520 (in the 500–560 target). **Medium (476) + Easy (355) BYTE-IDENTICAL** (verified). Aligns with Fable's FRACTURE tier; the old brutal Hard becomes the future opt-in RIFT tier (T8).
node --check clean. Reviewed (diff + risk surfaces + Agent A's live verification). HELD for the Wednesday batch. Note: gem size/rim aesthetic wants the owner's eyes on a real 60fps machine.

### build146 — Thumbnail transform fix: the REAL Golden Buzzer "broken thumbnails" cause  ✅ verified · HELD (?v=450, Wednesday batch)
Playtest-6 (reported twice). NOT a data/URL problem and not literally GB-only — a mis-parameterized Supabase image transform in `jukebox.js` `thumb()` (used by every cover + song row). It rewrote the storage URL to the `/render/image/` endpoint passing **only `width=400`**; Supabase's render endpoint does NOT scale proportionally from a lone width — it returns `width × ORIGINAL-height`, i.e. the art **horizontally squished** (a 1254² cover → 400×1254; a 3500² submission → 400×3500). `object-fit:cover` can't undo a squish, so every cover rendered as a narrow, zoomed, broken-looking slice — worst on the large/non-square user-submission art that dominates the **Golden Buzzer winners** shelf (which is why it got reported there, and why the earlier isolated test was fooled: the `?width=400` URL DOES return a 200 image — a distorted one). Fix: pin BOTH dims + `resize=cover` (`?width=<s>&height=<s>&resize=cover&quality=72`) so the endpoint returns a proper SQUARE center-crop (the cover boxes are all square) — and a huge 3500² source now transcodes to a cheap 400×400 instead of 400×3500. **Improves EVERY thumbnail in the game, not just GB.** Verified live in preview: GB rail decodes 400×400 square + `complete:true`, song-list 160×160 square, zero console errors; the `onerror` fallback chain (thumb → full `artwork_url` → branded initial) is untouched. `node --check` clean. HELD for the Wednesday batch (the `?v` bump busts the cached squished version on deploy).

### build145 — MP-reliability #3a: re-emit the outgoing challenge broadcast  ✅ HELD (?v=449, Wednesday MP batch — needs 2-account test before publish)
Playtest-6 MP diagnosis (Agent D): part of why "calling people in, sometimes they don't connect." `sendChallenge` fired the `challenge` lobby broadcast **exactly once** while `acceptChallenge` already retries its `challenge-ans` 6× (asymmetric) — so a single dropped `challenge` packet left the target never seeing the invite → a full 12s "No response." Now `sendChallenge` re-emits the challenge every 2s (bounded ~8s, inside the 12s timeout), self-cancelling the instant the target answers (`onChallengeAns` nulls `pendingOut` on both accept and decline), the challenge is superseded, or the match goes live. **Safe by construction** — idempotent on the receiver (`onChallenge` just re-sets `incoming[fromId]={mid}` for the same mid, can't double-open), no happy-path behavior change. This is the one HELD-safe fix from the MP diagnosis; the rest (bc:user `realtime.setAuth` + retry-till-SUBSCRIBED, the match-stage ready re-broadcast, room-start/start re-emits) touch timing/auth and need the owner's live 2-account test → bundled for the Wednesday MP batch. node --check clean.

### build144 — Friendly dead-track message (no more raw DOMException)  ✅ HELD (?v=448, Wednesday batch)
Public-run polish: a dead / HLS-only / corrupt / CORS-blocked track was already caught (the `play()` error handler shows a toast + returns to menu — it is NOT silent), but it surfaced the **raw** error — a user could see `"EncodingError: Unable to decode audio data"`. `bufferedProvider` now wraps the fetch+decode and rethrows a friendly, actionable message — **"This track can't be played right now — try another one."** — which the existing `play()` catch renders; the raw cause still `console.warn`s for debugging. Success path is byte-identical (the new catch only runs on a fetch/decode failure). Verified by trace (the rethrow lands in the toast at the play() catch) + `node --check`; HELD for the Wednesday sync.

### build143 — Hardening fleet: catalog economy + jukebox escaping (+ GB thumbnail investigation)  ✅ reviewed · HELD (?v=447, post-show batch)
Fleet pass under the orchestrator (one owner per file, off the live-show core). **catalog.js (economy/entitlements):** (1) `ownsItem()` now slug-normalizes hyphen↔underscore on ALL three ownership sources — FREE_ITEMS, Bonus-owned, and server entitlements (build100q only normalized the server branch), fixing a real "owned but can't play" where a FREE or Bonus-purchased paid level read as locked because `multiplayer.js` queries `ownsItem('level', entId)` in the underscore form; (2) `bonusBuy()` dedup is now slug-insensitive so the same item bought under a different id form can't Bonus-Sparks double-charge. Confirmed: the practice-run exclusion gate in `recordLocal` is untouched/intact, and no launch/MP path (`launchTrack`/`liveProvider`/`challenge*`) was touched (byte-identical for tonight). Reviewed clean: no play path mints cashable Sparks; `spendSparks` idempotency-keyed; failed runs stay out of the career aggregate; backend-gated shelves swallow 404 once with no console spam. **jukebox.js:** `flixsHero()` poster URL now passes the `safeUrl` scheme-allowlist (defense-in-depth; the one user-URL that bypassed it) — full XSS/escaping/brand/responsive audit otherwise came back clean.
**GB thumbnail report (owner):** investigated the "broken Golden Buzzer thumbnails." Root cause = the **flag-flip transient** — Lovable deployed the `golden_buzzer` flag mid-session, so the shelf lit up the instant the flag went live while the (1–3.5MB) covers loaded for the first time. Verified live: all 21 browsable winners' artwork loads with **0 errors** (both the 400px transform thumbnail AND the full-res; raw + transform URLs both 200 image), and the render paths already fall back to a branded initial for genuinely-missing art. **No code fix needed** — a refresh shows the real covers. HELD with the fleet batch; syncs after tonight's show.

### build142 — Combat shock frequency: SHOCK_COMBO_STEP 8 → 15 (owner playtest-5)  ✅ SYNCS NOW for tonight's show (?v=446)
Owner playtest-5, live MP with combat ON: "every ~8 seconds it felt like I was getting zapped — so much it felt like I wasn't even playing against someone." build133 had dropped the shock milestone 15→8 to make damage *land* in a casual duel, but 8 overshot — combos cross 8 constantly, so the rival got strobed. Back to **15**: a rival must earn more combo before each shock, so a zap reads as an earned hit instead of a constant stun. One-line constant in `multiplayer.js` (single source of truth — both the main-deck and tournament/spectator shock paths read it); the ~2.2s stun, the OD trigger, and the 4s receive-cooldown are unchanged. Needs the owner's live 2-account combat feel-check, but node-safe and low-risk. **Synced this pass** (with the held build141) so tonight's performance has the corrected combat cadence.

### build141 — Practice Mode (section loop + slow-down + restart-section)  ✅ reviewed · syncs with build142 (?v=446)
A real Guitar-Hero practice tool, single-player only (Explore-scoped as the highest-ROI non-MP delighter — Daily Rift already exists; Endless is heavier). Three parts: **section loop** (pick start/end mm:ss → only notes in `[start,end)` are charted, audio seeks to the start via WebAudio, and it auto-loops on section-end/wipeout so you can drill one passage), **playback speed** 0.5/0.75/1.0/1.25/1.5× (applied to BOTH the note approach and the `AudioBufferSourceNode.playbackRate`; pitch shifts with rate — accepted for drilling, no time-stretch lib), and **Restart-Section** in the pause overlay. Entry is a Practice panel on the song sheet (hidden for films/unplayable/demo/mock); a gold **PRACTICE** HUD tag + section/speed readout makes it unmistakable the run isn't scored. **Integrity (the hard requirement, verified):** a practice run NEVER submits to the leaderboard and never updates the per-song best, career/lifetime aggregate, Bonus Sparks, Daily-Rift ×3, or XP/streak — a single master `results.practice` early-return in `catalog.recordLocal` above all of those + the `session.submit` skip in `endGame` (an adversarial 2-lens Opus review confirmed the exclusion is airtight and off-practice is byte-identical). Note filter keeps the array time-sorted, keeps chords whole, and clamps hold tails at the section end. **Fast re-arm:** the loop reuses the decoded buffer + session (no re-decode, no loading-screen flash, no ~2.1s guitar-in, a snappy ~650ms "GO" instead of the full 3·2·1); the loop-vs-onended double-rearm race is guarded by an `_practiceArming` latch + `++_playGen` supersession + nulling `onended` before `stop()` (no double audio). Also fixed a latent DemoPlayer bug found in review (stale `_pauseAccum`/`_pausedAt` corrupted `getTime()` on a re-seek; the reset is a no-op on every non-practice path). node-checked; headless-verified (first launch = normal pre-roll, re-arm = no loading flash, exclusion gates intact, zero console errors). **Loop *feel* at 0.5×/1.5× on a real 60fps machine still wants the owner's ear.** HELD — folds into the next batch sync after the MP test.

### build140 — MP join reliability + symmetric ready + pre-start strings  ⚠ needs 2-3-account test · SHIPS THIS WAVE (?v=444)
Playtest-5's biggest cluster. Accepting a battle-call / auto-match often dumped the callee on the MAIN MENU (especially booting fresh from the website); symmetric READY could deadlock; the left rival deck showed "floating strings" during the 3·2·1. **Unified fix:** all four join entry points (`?mproom` deep-link, in-game ring ACCEPT, auto-match `qmMatched`, and the new `bc:user` broadcast) now funnel through **`_pendJoin()`** — it fast-paths on the host's `room-meta` (the happy path, byte-identical) then **DIRECT-joins the room channel by rid** after a ~10s grace, so a late/backgrounded host still converges instead of dead-ending on the menu. Plus: a **2s `room-ready` re-broadcast** (a single dropped broadcast could wedge "both READY → GO"); `qmResume`/challenge-accept **retry hardening**; and a **warm-dark neck panel** painted behind the left strings during the count-in (no more floating strings). `catalog.js` gains `challengeAccept`/`challengeDecline`/`roomStatus` authed wrappers.
**Opus 3-lens adversarial review → 4 CONFIRMED, 0 refuted** (the paths-regression lens found **ZERO** — show/spectate byte-identity is preserved; `_pendJoin` never touches the show-room path). All four fixed before ship:
- **A (high):** the `/room-status` `alive:false` bail was **unreachable** — the same-tick direct-join set `room.id` first, so a guest who accepted a truly-gone room was parked on "Waiting for the host…" forever. Preflight now fires early (tick 2) + gates the direct join on `!_roomDead`, and ejects (`leaveGuestRoom` + banner) if the async dead result races a join that already landed.
- **C (high):** leaving during a guest pend **leaked the tick interval** → the next tick re-joined and **yanked the user back** into the room they left (+ stray pings). `leaveRoomChannel()` now calls `_clearPendJoin()`, and the abandon guard was weakened to `(!_pendingRoomJoin && !room.id)`.
- **F1 (med):** the autonomous `bc:user`/notifications push had **no `_inMenus()` gate** → an inbound call mid-run pulled you off your song / tore down your live match room. Now **stashed and consumed on the next return-to-menus edge** (matching the poll's menus-only contract), fresh-bounded to 90s.
- **E (low):** `onRoomPeers` p1-heal could pick a **spectator** as the opponent (premature READY + wrong name; the match still started correct via host-broadcast ids). Specs filtered from both fallback passes.
**Backend-gated (degrades silently to the 30s poll today; needs Lovable):** the `bc:user:<uuid>` private-channel broadcast + the `notifications` realtime publication. The direct-join funnel, symmetric-ready, retry hardening, and neck panel all work now. Verified: `node --check` both files; combined A+B+C boot-smoke clean (zero console errors, MP open/close no throw). **The 2-client join/ready flows can't be proven headless — they land on the owner's preview for a 2-3-account test before publish.**

### build139 — Ember backdrop busier pass (more particles + rising-spark stream)  ✅ verified · SHIPS THIS WAVE (?v=444)
Playtest-5: "Malevolt amber drift needs more movement, more particles to feel busier." Enriched `drawEmber` in `procbg.js`: FAR 220→330, MID 120→180, NEAR 40→52 (lite 96/84/14); a new fixed **rising-spark pool** (≤64/26) lifting off the coal bed; per-particle lateral **swirl** on the FAR/MID drift; a livelier **two-sine coal flicker**. ~+250 populated elements + constant gentle upward motion. Honors every Backdrop-LAW constraint (verified in-diff + a 200-frame headless sweep, zero errors): **zero per-frame allocation** (all pools sized at reseed; draw loops walk fixed arrays with `continue`-cutoffs, never splice), `_soft` halves the rising-spark + FAR/MID draw ceiling, **reduce-motion freezes all new motion** (flicker→steady 0.5, rise/swirl gated), loop-seamless (`sin(t·π)` end-fade + mod-1 rise, no pop), brand-warm via `_warmHue` (no raw literals), bottom alpha capped so the lanes stay readable. Tier-up cutaway untouched.

### build138 — Local co-op dual input (2nd controller drives the LEFT deck)  ⚠ needs 2-controller test · SHIPS THIS WAVE (?v=444)
Playtest-5: in Local Versus both keyboard AND a 2nd controller drove the RIGHT (P1) deck; the LEFT deck was inert. Root cause: `pollGamepad()` routed EVERY connected gamepad into P1. Fix (`game.js` only): `_couchP2PadIndex()` returns P2's claimed gamepad index **only** when a live couch P2 shadow engine exists (null for single-player AND online MP — online MP is a network opponent, never a local `_p2`), and P1's pad loop skips exactly that index. P2's own `p2PollPad` pipeline + couch's left-deck renderer were already wired; `couch.js` needed no change. Safe failure mode: an unreadable P2 index returns −1 (no exclusion) — a no-op, never a break. Single-player + online MP byte-identical. **Needs the owner's 2-controller test** (can't press two physical pads headless).

### build137 — Album Release Parties shelf in Browse  ✅ verified · held for next sync (?v=443)
Owner wishlist (the pair to Golden Buzzer): the platform's album RELEASE PARTIES surfaced in the game. Adds a Browse "Album Releases" shelf — a horizontal row of album cards (cover art in a warm/gold frame, title, artist), each with a crimson **"LIVE THIS WEEK"** pulse when the party is live (else a gold "NEW ALBUM" tag) and an **"N of M playable"** line when only some album tracks are consented/decodable; clicking opens that album's playable tracklist. New `RhythmCatalog.releaseParties()` / `albumTracks(id)` fed by a new `GET /release-parties` endpoint (requested from Lovable). **Security:** album titles/artist names/cover URLs are user-authored, so all strings are `escapeHtml`'d and cover URLs pass a new `safeUrl()` scheme-allowlist (verified: a `javascript:` cover is rejected → fallback tile, a `<script>` title renders as escaped text). **Graceful:** the endpoint doesn't exist yet, so the fetch fires exactly once, swallows the 404 (no console spam / no retry loop), and the shelf stays hidden — Browse renders 100% normally until the endpoint lands. Row is `overflow-x:auto` (page never scrolls sideways); LIVE pulse static under reduce-motion; brand crimson/gold, no purple/blue/green. **HELD from sync with build135/136** — folds into the next version (combined adversarial review before that sync, with a focus on the escaping/safeUrl surface). Verified via dev-mock (renders/hides correctly) + against the real backend (single 404, clean console).

### build136 — Results-reveal cascade  ✅ verified · held for next sync (?v=442)
Audit finding #7: the solo results screen revealed everything at once (the ~900ms score/accuracy count-up, grade bloom, star stagger, judgment breakdown, badges all within ~90ms) so the eye couldn't follow the number climb. Now it CASCADES like a proper Guitar-Hero results screen: grade bloom (0→0.70s) → stars (0.50→0.86s) → **the score/accuracy tally now delayed to start at 0.42s** so the climb is its own beat → badges/blurb/summary rise in (0.40–0.55s) → the judgment breakdown bar rises + fills last (0.72s). Reads as one smooth ~1s downward sweep instead of a simultaneous pop. Pure reveal SEQUENCING — no score/grade/star/data logic changed (final values byte-identical). New `@keyframes resultsSectionIn` (fade+rise) with a staggered `animation-delay` ladder scoped to `#results.active`; a `html.rr-reduce-motion` override collapses everything to instant (no delayed count-up, no stagger — verified no blank/half-built state). MP verdict path unaffected (it uses `showWinner`, not `renderResults`). **HELD from sync with build135** — folds into the next version (a combined adversarial review runs before that sync).

### build135 — Golden Buzzer Winners shelf in Browse  ✅ verified · held for next sync (?v=441)
Owner wishlist: the platform's Golden Buzzer winners (best-of-the-best) deserve a home in the game. The per-track WINNER treatment was already built (gold ring + ♔ crown + "Golden Buzzer Winner" tag on the song list + coverflow + launch cheer) but dark because the backend never sent the flag. This adds the dedicated Browse home: a full-width **gold "Golden Buzzer Winners" hero card** at the top of Browse + a **"♔ Golden Buzzer" coverflow rail** alongside Featured/Hot/New/Surprise, both sourced from a new `RhythmCatalog.goldenBuzzerTracks()` (music tracks flagged `golden_buzzer`). Opus-built, focused adversarial review → **SHIP, 0 defects** (graceful-empty: renders 100% normally with zero winners — the card/rail/tab all hidden until the flag arrives; no Browse layout break; correct rail↔tab cache gating; brand-clean gold, no purple/blue/green; fully guarded, no throw). **Backend dependency:** the game-catalog `/tracks` response must return `golden_buzzer:true` per winner (a LEFT JOIN on `golden_buzzer_awards.submission_id` — requested from Lovable; ~32 winners are already consented/playable). Renders empty until then. **HELD from the current sync so the ?v=440 MP preview stays clean for the owner's 2/3-account test** — folds into the next version.

### build134 — MP spectator overhaul: real dual mirror-decks + LIVE-WATCH discovery  ⚠ needs 3-account test (?v=440)
The spectator view was broken (owner's screenshot: two static decks that didn't reflect the match — a synthetic reconstruction scrolling both decks on one shared clock, and show-room-only). Now a NORMAL-room watcher gets a full-bleed dual-deck stage where **each deck is driven by that player's REAL streamed progress + real hits/score**, so the two decks genuinely diverge and mirror the actual match. Both players now stream their render-frame whenever a spectator could be present (mobile included; verified send-only + byte-identical to the players — only an added outbound broadcast, `getRenderFrame` just drains an outbound-only buffer). Discovery: an open, in-progress room re-advertises `live`/`mid` every ~10s so a watcher opening the lobby mid-song sees a **"🔴 LIVE — WATCH"** card → late-joins straight into the dual stage. Opus-built, 3-lens adversarial review (player-identity / spectator-correctness / safety-brand) → **3 confirmed findings, all fixed** — none touch the players' game or the show-room path:
- **Verdict was occluded:** when a watched match ended, the full-bleed stage (z-252) stayed mounted over the "YOU WIN/LOSE" card (z-240), so the watcher saw frozen decks + no result, then got bounced ~12s later. Fixed: the normal-dual `final` handler now unmounts the stage before `showWinner()` so the pre-diff verdict card shows (honoring the spectate byte-identity decree; the show path's in-stage verdict is untouched).
- **Decode-failure hang:** if the spectator's chart failed to decode, `_specDeckFallback` unmounted the stage (killing the 12s watchdog) but left `_specDual` true, so the compact-panel 14s self-heal (gated on `!_specDual`) never re-armed → a watcher could hang with no exit. Fixed: re-gate the self-heal on `!_specStage` + re-arm `_specLastFeedAt` in the fallback so a stranded watcher bails within ~14s.
- **CAVEAT:** the real dual-stream render, the state-send reaching a watcher, and mid-match discovery can only be *proven* with 3 live clients (2 playing + 1 watching) — ships to PREVIEW for that test. The two players' own game is byte-identical.

### build133 — Playtest-4 MP fix batch A + local co-op fix  ⚠ needs 2/3-account test (?v=439)
Group multiplayer playtest (on live v437) surfaced a cluster; a line-numbered Opus investigation root-caused each, this batch fixes the code-confirmed ones (spectator overhaul is build134). Opus-built, 3-lens adversarial review (song+tournament / combat+seat+couch isolation / cosmetic+safety) → **0 code defects**. Tracker: PLAYTEST4_MP.md.
- **[P0] Normal challenge played the DEFAULT song** — `resolveAndStart` re-looked-up the picked track in the local catalog and fell to the demo on a catalog miss (same class as the build126 launch bug, never applied to the MP path). Now the pick's decodable `audio_url` rides the broadcast as a distinct `pickUrl` field (`_pickUrlFor`/`_songPayload`); on a catalog miss for a real track the resolver synthesizes a minimal record so the in-browser chart plays the RIGHT song. Demo sels still → demo; show rooms still route through the untouched `resolveShowStart` audio-security guard.
- **[P0] Tournament final spawned an empty round** instead of crowning — on a forfeit/leaver/host-migration edge a departed survivor's id inflated the round count. New defensive terminal guards (`_presentTourIds` + `hostCrownChampion`): `hostBeginRound` filters the alive set to present players and crowns immediately at ≤1; `hostFinishRound` filters winners+byes against the live roster and crowns if it collapses. Never crowns from an empty set; the real-final bracket math is unchanged.
- **[P0] Combat/damage never fired in a casual match** — the damage "shock" only triggered at a **combo of 15** (or Overdrive), so normal-length combos never reached it. Lowered `SHOCK_COMBO_STEP` 15→**8** (fires at combo 8; OD trigger + 4s cooldown intact). Plus a **new host-only in-room COMBAT toggle** (`#mpx-room-combat-toggle`) — combat was only settable at room creation before; now the host can flip it in the waiting room and it's adopted on the next match via the existing broadcast.
- **[P1] Accept-seat UX** — a normal room already auto-seats the first joiner, but the copy was confusing (leftover artist "call-in" language). Clarified: joiner sees "You're in — hit READY"; host with a seated opponent sees "<name> joined — tap READY to start the versus"; empty → "Waiting for a player — share the room code." No seating-logic change; the artist/show-room path is untouched.
- **[P1] Now-playing song display** — the track title was in a panel hidden during versus mode; added a **top-center `.vs-nowplaying` chip** (song · artist) in `mountVsHud`, shown in 1v1, rooms, and tournaments, removed on teardown.
- **[P2] MP "Play Now" hero** — the COMBAT badge sat directly over the "QUICK MATCH" eyebrow; repositioned it (top-right, under RANKED). No overlap.
- **[P2] PERFECT sound** — the bright accent tick I added in build129 was too piercing (triangle 2100→1400Hz); softened to a warm low **sine 820→560Hz at ~40% gain**. The crisp visual PERFECT flash is unchanged.
- **LOCAL CO-OP (2 controllers) fixed** — P2's half was blank because couch.js created the split-screen elements with the canonical IDs then **renamed** them, so the vs-mode layout CSS never matched and P2's canvas stuck at 300×150. Now uses the canonical IDs (mirroring online MP's `mountVsHud`); P2's deck renders full-half. P1 byte-identical when co-op is off.
- **CAVEAT:** the song-sync, tournament-crown, combat-landing, accept-seat flow, and 2-controller co-op can only be *proven* with 2–3 live clients — ships to PREVIEW for that test before publish.

### build132 — Fix: store/skin name misspelled "Sasoka" → "Sisoka"  ✅ verified (?v=438)
Owner: the featured bass's name is spelled **Sisoka**, not "Sasoka". Fixed both user-facing DISPLAY titles (the skin picker entry ~index.html:8108 and the store card ~index.html:10438). Left every internal `sasoka` id, `item_id`, asset path (`assets/guitars/sasoka.png`, `sasoka-card.jpg`, level covers/loops), and the `SKIN_GUITAR`/`SKIN_LEVEL_BG`/`STORE_FEATURED`/`RR_FINISHED_LEVELS` keys UNCHANGED — renaming those would break entitlement/ownership matching + asset loading. Grep-verified: 2 display titles → "Sisoka", all ids/paths still `sasoka`.

### build131 — P0: hosting MP from the live panel dropped to SOLO — now a real open versus  ⚠ needs 2-account test (?v=437)
Owner live-playtested with a group: site "Play in Reactive Rhythm" → host multiplayer → "start match" ran SINGLE-PLAYER even with a friend in the room; the friend couldn't take a seat and saw a broken (dark, letterboxed, washed-out) deck. **Root cause (one chain):** the site GO-LIVE flow opens a SHOW room whose challenger seat defaulted to `'artist'` (artist-only, manual ACCEPT), and the auto-seat that would let a friend in was disabled behind `?dev=1`. So `room.p2` stayed null → START read "▶ START (SOLO)" → `_soloRun` → solo run; the unaccepted friend was demoted to the (broken) spectator deck. A recurrence of the playtest-3 handoff cluster on the newer show-room surface. Opus-built (multiplayer.js/catalog.js/game.js/index.html), 3-lens adversarial review (state-machine / isolation / safety) → **0 code defects** (the host can't get wedged; nothing leaks into normal rooms; the artist-call-in path is intact).
- **Open, auto-seating seat by default:** the host-from-panel seat policy now defaults to `'auto'` (first-come) and the auto-accept fires without `?dev=1`, so a friend who joins is auto-seated as `room.p2` → `_soloRun` false → **real versus split-screen**. The original artist can still take the seat (it's now a superset, not artist-exclusive); `'confirm'`/`'artist'` policies remain selectable.
- **Solo-start guardrail (no silent solo):** an open+empty seat shows **"… WAITING FOR A CHALLENGER"** (primary disabled) with a **COPY ROOM LINK** + a low-key **"Start solo anyway"** escape hatch; a seated seat shows **"⚔ START VERSUS"**; the `'artist'` legacy policy still shows "▶ START (SOLO)". A host can no longer *accidentally* start solo when hosting a versus (new `_showSeatOpenEmpty()` + `_showSoloArm`, gated to show rooms only — never leaks into normal/battle rooms).
- **Spectator deck fixed:** genuine watchers get a full-bleed, legible deck (solo-watch deck 62vw letterboxed → 88vw; guitar-art/notes/strings alphas raised out of the washed-out range). Engine still never runs on a spectator; `!spectating` byte-identity preserved.
- **Discoverability:** open, not-yet-full show rooms re-advertise to the lobby every ~12s (stops on seat-fill / go-live / close) so others can actually find + join them from the browse list.
- **`GoldenState — null`:** the HUD track label (game.js:2892) now shows just the title when the artist is null, mirroring the results-label guard.
- **HARD CAVEAT:** the realtime seat handshake, the real versus split, and the live spectator render can only be *proven* with two signed-in clients — this ships to the PREVIEW for a 2-account test (host → 2nd account joins → auto-seats → START VERSUS → both play → verdict) BEFORE publish.

### build130 — Fix: MP end-of-round "YOU WIN" scorecard was cramped/broken  ✅ verified (?v=436)
Owner reported the multiplayer round-end verdict card rendering broken: each player's stat line (`63.1% · 22x · C`) was squeezed into a narrow vertical pill, wrapping every token onto its own line (accuracy / dot / combo / dot / grade — five cramped lines). **Root cause:** `.mpx-scorecard` lives inside `.mpx-step-winner`, a flex COLUMN with `align-items:center`, which collapsed the scorecard to its min-content width (~106px) instead of stretching — so the two `flex:1` plates shrank to ~30px and the meta wrapped. NOT a build128 regression (that touched a different in-game HUD plate); this was a latent layout bug. **Fix:** `.mpx-scorecard { width:100%; max-width:460px; margin-inline:auto; }` gives the plates real room (measured 207px each at desktop, both stat rows now on ONE line) while staying centered; no forced `nowrap`, so genuinely narrow phones still wrap gracefully instead of overflowing. Pure CSS on the shared `showWinner()` card — applies identically to spectators (byte-identity decree honored). Verified live via computed-style + bounding-box probes (screenshot times out on the animated MP canvas, per the project's stated method); console error-clean.

### build129 — Polish wave: game-feel + hot-loop perf + catalog resilience + UI consistency  ✅ verified (?v=435)
From a read-only adversarial Opus audit of the whole engine (POLISH_AUDIT.md, 15 ranked findings). Twelve landed here across 3 disjoint files (game.js / catalog.js / index.html), each built on Opus, then a combined 4-lens adversarial review (incl. a lens dedicated to *breaking* the two hot-loop changes) → **0 findings, clean**. Also shipped: **build125 WEEKLY RIFT** (below).
- **Game-feel (game.js):** the combo number now **ticks** (a subtle per-hit pulse — it was frozen between tier milestones, missing the genre's core dopamine signal); big earned callouts (GOLDEN MODE / OVERDRIVE READY / CLUTCH / STREAK) **stop getting stomped** by the next note's GREAT (a priority guard; MISS still interrupts); **breaking a big streak (≥50) now reacts** — a "STREAK BROKEN" callout + heavier shake/glitch/squelch (a small combo-2 whiff is unchanged); **every non-failed finish gets an audio sting** (B/C/D runs were landing silent under the count-up); and **PERFECT gets a distinct signature** — a whiter core-flash + a short bright accent (the word stays suppressed, so precision finally *feels* better than a sloppy GREAT). No scoring/timing/note-order touched — all additive feedback, reduce-motion gated.
- **Hot-loop perf (game.js), behavior-preserving:** the per-frame note render loop now **breaks** at the first off-screen-future note instead of scanning the whole array (byte-identical output; notes are time-sorted so it's safe), and miss-detection uses a **monotonic cursor** that starts past already-judged notes instead of re-walking the full array every frame. Both proven not to change what's drawn or when a miss registers (verified: 57 misses register correctly across repeated passes).
- **Catalog resilience (catalog.js):** a single flaky page mid-crawl no longer **throws away the entire already-fetched live library** and drops everyone to 1,000 mock songs — each page is now guarded, a partial crawl is kept (mock only when ZERO real tracks load), and `catalogLive` stays true so server-charted launches aren't blocked all session. Plus **parallel page loads** via X-Total-Count (order-preserving, `push` not `concat`) with a sequential fallback (the live API currently returns `X-Total-Count: null` to JS, so prod takes the safe sequential path today and still loads the full 1,171-track library — the parallel path auto-activates if the backend exposes the header).
- **UI consistency (index.html):** the two differently-styled difficulty pickers now share ONE treatment (Chakra Petch + uppercase); Settings control labels unified onto Chakra Petch (mono reserved for data/keycaps); "Sign in" casing normalized across the auth flow; a typo'd toggle label ("Level's" → "Themed"); and an unused Bungee font dropped off the render-blocking font fetch.

### build125 — WEEKLY RIFT ladder — Sunday-reset weekly challenge card  ✅ verified (?v=435)
A delighter for the Sunday-night ritual: a gold hub card showing **this week's Rift track** (a deterministic weekly pick, same for everyone, rotating each Monday), **your best-of-week**, a **countdown to the Monday 00:00 reset**, and a PLAY THE RIFT button. Self-contained `RhythmWeeklyRift` ledger (goals.js — ISO-week key, rollover guard mirroring the retention ledger, single localStorage key, an optional signed-in submit seam for a future shared weekly board). Records a run via an **arm-on-launch → consume-at-results** flag that can't leak into a non-weekly run (records only non-failed matching-track runs), firing a gold **"NEW WEEKLY BEST"** results badge. The countdown ticker is a single hub-gated interval (no leak). Local personal-best only — no fabricated cross-player rank. Every risk seam hand-verified; nothing throws into scoring.

### build128 — MP VS-bar "friction front" redesign + score-plate fit  ✅ verified (?v=434)
Owner: the top MP "who's winning" bar needed to POP — contrasting colors that battle, sparks/embers where they meet, glow, more prevalence; AND the MP score numbers were "bleeding off" their UI box. Opus-built (multiplayer.js + index.html only), 3-lens adversarial Opus review (perf / correctness+spectator-safety / brand+mobile) → **0 findings, clean**.
- **VS-bar → the friction front:** flipped to **YOU = crimson, RIVAL = gold** — two high-contrast brand colors that clash — carried through the fills, pole chips, in-bar labels, and each side's score number (matching-color text glow). The seam between them is a **white-hot molten core** (cached radial sprite) that breathes/flickers with how close the match is, and **sparks grind off the seam** additively as glowing embers that cool to each side's tint.
- **Intensity = closeness:** emission scales on `(1−|lead|)²` gated on both streams actively playing — a **dead heat throws a burst of hot sparks every frame**, a **blowout goes calm** (core shrinks/dims, no sparks) so it's dramatic when it should be and quiet when it shouldn't. A **lead-flip fires a 22-particle white-hot detonation + bar jolt + knot flare** — a satisfying crack when momentum swings.
- **Perf/robustness:** fixed **40-slot spark pool**, the molten sprite is **built once and cached** (`_tugCoreBlob`) — **zero per-frame allocation** (frame path grepped clean); sleeps fully when no sparks and no core; **fxLite** = spark canvas never mounts; **reduce-motion** = all spark/core/jolt/flip gated off, bar degrades to a clean static two-color lerp. No scoring/chart/timing/hit-detection touched (diff confined to the tug FX + score-plate CSS).
- **Score-plate fit:** the plate used `min-width:172px` on a content-box, so a 7-digit comma score's `nowrap` intrinsic width grew the plate to ~204px and **stretched the frame PNG 18% off its runway** = the "bleeding off." Fixed to `width:200px; box-sizing:border-box` (frame never stretches) + centered `tabular-nums` digits `clamp(15px,15cqi,28px)` auto-fitting the 168px inner window. **Measured proof:** "9,999,999" renders 114px inside 168px, plate scrollWidth (200) ≤ clientWidth (200), value not clipped; mobile branch unaffected.

### build127 — VFX backgrounds: depth + composition step-up (dawn / weave / ember)  ✅ verified (?v=434)
Owner: the three Tier-I reactive backdrops read "completely lackluster… a lot of empty room" — flat single-layer scatter on black. The fix is **composition + depth (fore/mid/background + parallax + a sense of place), not particle count.** Opus-built (procbg.js only), 3-lens adversarial Opus review (perf/allocation / correctness / brand+Backdrop-LAW) → 2 candidates, **both refuted, 0 real**.
- **DAWN (First Light):** the dead upper sky now reads as a living dawn — 4 stratified warm sky **bands**, high drifting **aurora ribbons**, distant twinkling horizon **city lights**, a volumetric sun **atmosphere halo**, and two **parallax ridge silhouettes** (far hills + near jagged skyline) that slide opposite the star-scroll for real depth.
- **WEAVE (Steady Hands):** the lonely horizontal strings became a real **LOOM** — vertical **warp threads** (a near bright pass + a far dim ghost pass) crossing the weft with a perspective fan, glowing **intersection nodes** pulsing on beat/combo, over a cached woven-cloth **haze**.
- **EMBER (Ember Drift):** the embers got a **source and volume** — a glowing forge **coal bed** along the bottom (per-coal flicker), layered rising **heat-haze columns** (real smoke volume, seamless mod-1 rise), and an ambient **forge-glow wash** so the whole scene feels lit by the fire. Depth reads far faint sparks → mid smoke → near bright embers.
- **Constraints honored:** all new elements loop-seamless + calm at rest (drama still spikes only in the tier-up cutaway), warm-only (audited: 0 forbidden hues, every literal crimson→amber→gold), **no per-frame allocation** (every pool grows only inside its `_state.dirty` reseed), and all drift/parallax/rise/sway freeze under reduce-motion; expensive layers shed under `_soft`. The builder self-caught + fixed 2 var-hoisting bugs (ember `frozen`/`coalSpr` used before declaration → silent visual dead-spots) pre-commit.

### build126 — Fix: host live-panel launch played the DEFAULT song, not the picked one  ✅ verified (?v=434)
Owner watched a host on the website's live project panel pick "Play in Reactive Rhythm" and the game opened the **default demo song (LUNAR WAVES)** instead of the selected track — a trust-eroding P0. **Root cause = a boot race:** both launch sites (`launchTrack()` and the jukebox sheet play handler) gated the in-browser-chart path on `catalogLive` being true, but a review/host-launched track routes *during* the catalog crawl — before `catalogLive` flips — so a perfectly resolvable track fell through to `playDemo()`.
- Fix: route on a **catalog-independent** `isLaunchable(track)` (has a decodable, non-HLS `audio_url` and isn't the demo) at BOTH launch sites, instead of the `catalogLive && id!=='demo'` gate. A resolvable picked track now always charts + plays itself, whether or not the full library has finished loading.
- Review fix (both launch twins must match): the jukebox sheet path was missing the `(catalogLive || track._review)` guard its `launchTrack` twin had on the *server-chart* branch — restored it, so a mock/sample row in preview/mock-fallback mode (also a transient-backend-outage state) still falls back to the demo instead of erroring-and-bouncing.

### build122 — Delighters: the retry/return loop pack  ✅ verified (?v=433)
Net-new "outside-the-box" features (from DELIGHTERS_SHORTLIST.md) — the cheap, high-reach hooks that pull players back into a retry and back tomorrow. Opus 2-lens review; both lenses converged on one real bug (fixed) + 2 cosmetics (fixed).
- **CLUTCH** — rebuild a broken combo back PAST where it snapped and the neck erupts crimson with a "CLUTCH ×N" callout (reuses the combo-cutaway FX + shake). The genre only ever celebrates going UP; nothing rewarded RECOVERING — this is the loss-aversion flip. Gated on a real prior peak (≥25). **Purely observational — never touches score/combo** (verified read-only). Surfaced as a results badge.
- **ONE NOTE FROM GLORY** — a run that *barely* misses Full Combo or a grade-up gets a dedicated results stamp ("1 NOTE FROM FULL COMBO", "0.4% FROM AN A") + a pulsing instant-replay button — the near-miss ache that drives the retry.
- **COME BACK TOMORROW** — surfaces the streak-*freeze* that already existed but was silent: a "STREAK SAVED" reveal when a freeze rescues your streak, plus a gold ribbon ("Streak: N days — play 1 song tomorrow to keep it") with a `.ics` add-to-calendar reminder.
- **Review fixes:** CLUTCH covered only 2 of the 3 combo-break sites (the early-hold-drop leaked a stale peak → could spuriously arm a CLUTCH) — mirrored the exact reset. Near-miss "0.0%" → "<0.1%". `.ics` DTEND==DTSTART (zero-length event strict clients drop) → day-after. The builder also self-caught a critical parse-break (an escaped apostrophe that would've killed the whole menu IIFE). Brand/parse/reduce-motion all verified clean.

### build123 — Performance: MP rival-guitar cache  ✅ verified (?v=433)
Opus-built + **byte-verified identical** (empirical getImageData diff: 0 differing channels / 768,000, maxDelta 0). MP `renderGhost` re-sliced the warped rival guitar into ~40 drawImage bands every frame on a second rAF; now baked once into an offscreen canvas and blitted once/frame (invalidation key covers skin/size/scale/warp/anchors/placement). **Note:** the two bigger single-player backdrop caches (warped-guitar re-slice, drawComboEnergy feather) were attempted and byte-diff-PROVEN to drift ±1–2 (they composite semi-transparent/AA layers over a per-frame-animated backdrop, unlike the opaque rival guitar), so they were correctly NOT shipped — they're only viable if sub-pixel edge drift is acceptable (owner call, see PERF_FINDINGS.md).

### build124 — RIFT REPLAY: the shareable 6-second clip  ✅ verified (?v=433)
The highest-ceiling virality lever (DELIGHTERS_SHORTLIST idea #2): share.js only made a static PNG of numbers — nobody shares that; they share a *clip of a sick run*. `replay.js` (new `window.RhythmReplay`) records the run's hottest ~6 seconds (peak-combo window) into a bounded ring buffer (78 frames, downscaled, evicted-canvas cleanup — no unbounded growth), then on the results screen a branded "▶ SHARE YOUR RIFT" button produces a branded VERTICAL clip — `MediaRecorder`/webm → animated-GIF (self-contained GIF89a/LZW encoder) → PNG-still fallback chain, feature-detected, never throws — and hands it to native share. Browser-verified: produced a real 2 MB webm (vp9), a 105 KB GIF, a 173 KB PNG. Wired via 5 fully-guarded engine hooks (init/tick/reset/offerOnResults); the integration caught + fixed a load-order bug that would have left the whole feature silently dead (armed:false). No scoring/timing touched. **Final feel needs the owner's eye + a real share test.**

### build121 — Playtest-4: performance — zero-behavior-change hot-loop opts  ✅ verified (?v=432)
Addresses the "performance under heavy load" complaint. A 3-lens read-only audit of the per-frame hot paths (render loop / backdrop / MP) classified findings SAFE_NOW (trivially-correct, zero pixel change) vs NEEDS_MEASUREMENT (bigger wins, unmeasurable headless → documented in PERF_FINDINGS.md for the owner's FPS meter). This ships the six SAFE_NOW opts (commit 9572ddb); each proven identical-output, verified by running a demo to `playing` with chart-stats intact + console clean.
- **`fretGeom()` memoized per frame** — it was recomputed 3–5×/frame (identical geometry) allocating a rect + 2 arrays each time; now a frame-scoped memo (`_fgFrameCache`, nulled at loop-top + render-end, bracketing the whole synchronous tick so inputs can't drift). Dev-hook/handler call sites still compute fresh.
- **Vignette radial gradient cached** — the static full-screen gradient was rebuilt every frame; now cached in a module var, invalidated on cw/ch change + resize. Same fillRect, identical pixels.
- **`new Path2D()` per lane → direct ctx path** — removed 5 heap allocations/frame in the string-glow (the double-stroke works against the current path identically).
- **Camera-shake translate skipped at rest** — below a 0.01 threshold the shake offset is exactly 0, so the per-frame `Math.random()` translate was a no-op; now skipped (save/restore balance untouched).
- **Ember decay `pow()` hoisted** — `Math.pow(0.18, dt)` ran per-particle (~760×/frame) but dt is loop-invariant; hoisted to 1 call/frame.
- **O(1) particle removal** — dead-particle culling used `splice(i,1)` (O(n) shift each) + `splice(0,drop)` cap-trims; now a forward two-pointer compaction (draw-order preserving, unlike swap-and-pop) + `copyWithin` cap-trims. Zero splice, one O(n) pass.
- **NOT touched (held for the owner's FPS meter, see PERF_FINDINGS.md):** the warped-guitar 64-band re-slice → offscreen cache (likely the biggest heavy-load win), the note-loop active-window cursor (O(total)→O(active), regression-sensitive), the drawComboEnergy feather-mask cache, and the MP renderGhost cache. These need on-machine measurement + (for the note cursor) a playtest sign-off.
- Verified: `node --check` clean on game.js + procbg.js; demo reaches `playing` with `__rrChartStats` populated; press/release/OD exercised across all 5 lanes; resize exercised (vignette invalidation); procbg ticks clean across dawn/weave/ember (hoisted decay path); console error-clean. No pixel changes — pure CPU/GC reduction, should help most on lower-end / integrated-GPU machines.

### build119 — Playtest-4: game-side moderation ENFORCEMENT (wired to the live backend)  ✅ verified (?v=431)
Completes the owner's "report → admin controls to suspend / warn / notify, with severity" ask end to end: the backend (rr_reports / rr_sanctions / rr_notifications + admin console + edge routes) was already built & deployed on Lovable; this wires the GAME to it. Three commits (422ecfc/f5f93b2/7abf776), then a 2-lens gating-sensitive review (1 MEDIUM → fixed).
- **Reports go live (catalog.js):** `submitReport` flipped from the localStorage-only stub to a live `POST /game-catalog/reports` (authed bearer, same pattern as /plays), with the localStorage queue kept as an offline fallback + a `flushReportsQueue()` that drains it on a confirmed sign-in (drop-on-success, reentrancy-locked — no lost reports, no double-POST). Added `getSanctions()` (GET /sanctions/me) and `ackNotice(id)` (POST /notifications/:id/ack), both fail-soft to null/no-op.
- **Enforcement (RhythmSanctions module, index.html + multiplayer.js):** on boot (after auth) and on entering MP, the game reads `/sanctions/me` and enforces — `blocks_all` (ban / full suspend) → a brand full-screen suspend gate (non-dismissible, capture-phase input block); `blocks_social` → MP entry blocked (solo untouched) via `recheckForMP()` consumed by `onActivated()`; `mute` → a flag + a non-blocking "you're muted until X" hint; each `pending_notice` → a severity-toned dismissible modal that acks on dismiss (so warn/notify persist until seen). All admin-authored text is `textContent`/`esc()`'d (no injection).
- **Chat-mute (chat.js):** `sendLobbyMsg`/`sendRoomMsg` drop a send (never reaching the channel) + show the mute hint while an unexpired mute is active; expiry re-checked at send time; fail-open if the module is absent.
- **THE SAFE DEFAULT (verified twice):** an unsanctioned player — `getSanctions()` returns null (signed-out / no sanction), the normal case for ~everyone — is BYTE-IDENTICAL to before: no gate, no modal, no dropped input, no blocked MP, no dropped chat. A `getSanctions()` error (offline/401) also creates nothing. Every enforcement path fails OPEN.
- **Review fix (MEDIUM):** the capture-phase keydown swallow keyed off `blocks_all` but the visible gate keyed off `blocks_all && suspend` — a backend response with `blocks_all:true` but `suspend:null` would kill the keyboard with NO gate on screen (a silent input trap). Now the gate shows whenever `blocks_all` is true (generic "restricted by moderation" fallback when no suspend record), so the input-block and the visible gate can never diverge. Verified live: the edge case now paints the gate ("Account Suspended · permanently") instead of trapping input.
- Verified headless (rr-verify): safe default byte-identical (no DOM created), simulated suspend/mute/notice all enforce + ack, chat-mute drops a muted send, `node --check` clean on all touched JS. **The real 2-account flow (owner sanctions a user via the admin console → victim's client sees the gate) needs the owner's live test.**

### build118 — Playtest-4: VS-bar clash sparks + cosmetics evolution + leaderboard redesign  ✅ verified (?v=430)
Three clusters, built as three commits (13f9cf6/cd0d82d/8467954), then a 3-lens adversarial review weighted at the real risk surfaces (XSS, the XP ledger, MP backward-compat). Lenses A (cosmetics/XSS/ledger) and B (MP/sparks) came back CLEAN; lens C (leaderboard) found 1 MEDIUM + 3 LOW → all fixed.
- **VS-bar clash sparks:** the head-to-head VS bar now throws a spark/clash burst at the meeting line, intensity scaled to how close the match is, plus a knot squash-stretch + brighten-on-blowout. Reuses the fixed 24-slot particle pool (zero per-frame allocation), gated behind fxLite + reduce-motion; scoring untouched.
- **Cosmetics evolution — the retention lever:** name COLORS + name FONTS + emblem/frame tiles now render REAL previews (sample text in the actual color/font, the actual emblem art) grouped into labeled sections, plus a prestige TITLE (⚡ RIFT LEGEND) at L20 **and** a 100-day streak. The big win: a shared `RhythmCosmetics.renderPlayerName()` broadcasts equipped cosmetics over MP presence + chat, so your grind is finally **visible to other players** at the lobby roster, chat, and VS-bar — not just vanity-to-self. Missing emblem PNGs (owner is generating them) degrade cleanly via `onerror`; nothing equipped = plain name.
- **Leaderboard competitive redesign:** the real "unknown track" share bug fixed (podium 'rankings' cards fell to the stock "LUNAR WAVES" demo name when a row's cached title was empty → fresh catalog re-lookup at share time + a "My Run" fallback, never "unknown"); a difficulty/track stale-response race guard; a three-way auth-state banner (signed-out / signed-in-empty / error); and rank ▲/▼/– delta glyphs (gold up / crimson down / chrome flat) + an "X pts to pass <name above>" chase card + a NEW PB pill.
- **Review fixes (lens C):** *MEDIUM* — a mid-flight sign-in could let a stale `authBanner` async-resolve stamp a "showing your local progress" banner over a now-live board; added a `_lbLiveShowing` guard in the banner choke-point. *LOW×3* — the Levels tab (local-only, no live backend) no longer shows a misleading "sign in to enter live rankings" CTA (neutral copy instead); the rank-delta snapshot is now LRU-capped at 50 board-keys (was unbounded across ~850 tracks × difficulties → localStorage-quota risk); rank deltas key by stable id / unique name and skip ambiguous/duplicate names (was name-only → wrong arrow on collisions).
- **XSS/ledger verified safe:** every peer-controlled cosmetic field is `esc()`'d + whitelisted via `byId()` (a malicious `name_color`/`flair_id` can't inject); the prestige grant only READS state (never re-enters the XP rollover guard) and is idempotent. Verified headless: all 5 changed JS files `node --check` clean; Levels banner neutral; console clean; no purple/blue in new colors. **2-client MP cosmetics + spark feel need the owner's eye.**

### build117 — Playtest-4: visual front door + VFX-levels density  ✅ verified (?v=429)
The first things the owner sees at login, plus the level backdrops. Built as three commits (two builders on disjoint files, in parallel), then two 2-lens adversarial reviews (procbg: 3 findings → all fixed; index: 1 MEDIUM → fixed).
- **Daily-goal card — premium makeover (p1):** the flat card became a hero — crimson border + inset gold hairline + chrome top-sheen, a focal flame with per-streak-tier escalation (SPARK→KINDLING→BONFIRE→WILDFIRE→INFERNO/UNDYING glow ladder, gentle breathe at the top tiers), gem-style progress pips with a crimson pip-pop on completion, a floating "WELCOME BACK" chip, and a "PLAY A LEVEL →" CTA that funnels into Campaign.
- **Roast copy (p1):** the flat "see you tomorrow" is now a 16-line rotating **roast** pool ("go work out those fingers"…), picked **deterministically per day** via `RhythmCatalog.hashStr('goalroast|'+date)` (mirrors the Daily-Rift seeded pattern — no `Math.random`, stable within a day, rotates across days).
- **Start-screen embers (p2):** consolidated three uncoordinated particle systems into one refined flame field (3-stop heat gradient white-hot→ember→crimson, flame-lick silhouette, age-shear), added the missing reduce-motion/fxLite gating, and fixed a real bug where the periodic canvas-resize timer wiped the static reduced-motion frame with nothing to repaint it.
- **VFX-levels density pass (p3, procbg.js):** the owner said the backdrops felt empty/under-reactive vs the old VFX. Density now scales with **live audio** (quiet thins, loud fills) on all three Tier-I renderers, base population is up (dawn stars 70→140 + an 80-star parallax layer, ember FAR 140→220, weave dust 40→90 + a sub-harmonic string layer), and the tier-up shockwave is restored into weave for parity. Base-density floors raised (0.5→0.62/0.68) so the resting backdrop reads **full** and audio adds the top ~32-38% (honors the Backdrop-LAW: stable base, drama in the cutaway).
- **Review fixes:** *procbg* — (a) a11y: the new parallax starfield now freezes its scroll under reduce-motion (main stars only twinkle in place); (b) PERF: ember's `_soft` slow-machine relief was **dead in-session** (it only applied at reseed, which never re-fires) → moved to a per-frame draw ceiling (`_densCap`) so relief fires the instant a machine slows; (c) the new weave sub-harmonic layer pushed it over the ~1200 path-op budget with no degrade path → now half-res (`SHSEG`) and `_soft`-gated. *index* — the new `overflow:hidden` on the daily card clipped the floating "WELCOME BACK" chip in half → removed (the flame glow's visible radius stays inside the card at every tier, so nothing needed clipping).
- Verified: `node --check` clean; procbg ticks 80×/renderer (normal + `_soft`-tripped) with zero errors + no per-frame allocation; roast pick confirmed deterministic across 7 sample dates; daily-card overflow now `visible`; no purple/blue in new colors. **Final visual taste (flame silhouette, card premium-ness, backdrop density/feel) needs the owner's 60fps eye — headless can't judge aesthetics.**

### build116 — Playtest-4: MP features (PvP/PvE + damage, room clarity + chat everywhere, matched settings)  ✅ verified (?v=428)
Three MP feature gaps from the playtest, built as three commits, then a 2-agent adversarial review (1 CRITICAL + 1 HIGH → both fixed):
- **PvP / PvE (damage on/off) + lightning:** a combat toggle surfaced in the lobby + room-create + tournament setup (host-authoritative, broadcast in `start`; default PvE/off), and a crimson **lightning-bolt VFX** (`mpZapHit`, reuses the combo-milestone bolt, reduce-motion gated) that fires on the victim's screen only when a rival lands damage — never in PvE.
- **Room-hosting clarity:** "HOST A ROOM / Open room · anyone can join · 1v1" is now a promoted quicklink + drawer row (distinct from Tournament) routing to room-create — testers kept diving into Tournament thinking that was how to play a friend.
- **Chat everywhere:** the existing text chat (chat.js) now mounts in **tournament** mode too (hidden during a live round, restored between rounds), alongside lobby + room.
- **Matched settings (fairness):** a room FREE/MATCHED toggle — in MATCHED, both players' scroll-speed/fail/chart are transiently forced equal (never persisted) so one player's slow-scroll can't be an edge; each side also sees the opponent's settings + a "MATCHED SETTINGS" chip on the VS bar. Scoring is unchanged (scroll only affects note fall, not the point formula).
- **Review fixes:** *CRITICAL* — a REMATCH inside a MATCHED room re-snapshotted the round-1 FORCED values (scroll 1×, fail off) as the player's "original", so after leaving they'd be **permanently stuck** on those settings. Now `resetForRematch` restores the real snapshot before the rematch re-snapshots (mirrors teardown; both seats). *HIGH* — tournament chat was hidden every round but only re-shown on the non-final await path, so it stayed gone through the champion screen; now restored in `onTourChamp`. (Two minor non-triggerable/cosmetic findings — mobile matched-HUD paint, MATCHED not remembered between room-creates — deferred.)
- Verified headless (rr-verify-b :8791): combat toggle round-trips + badge updates, `mpZapHit` no-ops under reduce-motion + can't fire in PvE, room quicklink → room-create (not tournament), tournament chat mounts, `applySettings({transient:true})` leaves `rr_settings` byte-unchanged; `node --check` clean. **The 2-client aspects (damage rendering on the victim, settings broadcast round-trip, tournament chat with a 2nd human) need the owner's 2-account test.**

### build115 — Playtest-4: sign-in handshake (game-side) + leaderboard bug fixes  ✅ verified (?v=427)
Two clusters. 2-agent adversarial review (1 MEDIUM + guidance → fixed).
- **Sign-in handshake (game-side, no site change):** confirmed by reading the site repo that the site + game are the SAME Supabase project (`bxiejoktoknybpraxebm`), the site stores its session in plain localStorage with the DEFAULT storageKey, and the game runs same-origin — so the session is already in shared storage. Added `_adoptSiteSession()` (catalog.js) that reads the `sb-…-auth-token` and calls `supa.auth.setSession()` on boot before `refreshAdmin`/`getUser`, front-loading the in-memory session so `getUser()` resolves the signed-in user instead of hitting the known boot-time session-restore race that showed "(guest)". Idempotent, fully guarded (a bad token can't break boot), exposed as `RhythmCatalog.adoptSiteSession`; the sign-in gate nudges it. **Caveat:** supabase-js *should* auto-recover a same-origin default-key session anyway, so this is belt-and-suspenders for the timing race — a `console.info('[rr-auth] adopted site session …')` diagnostic was added so the owner's live signed-in-on-site test reveals whether it fires. **Needs the owner's real-site verification** (a fake JWT can't validate the full handshake); if still guest, the root is deeper in the getUser/profile fetch and I'll dig in with the live signal.
- **Leaderboard bugs:** (a) the Easy/Medium/Hard toggle did nothing → moved it to its own row (Global + By-Song), wired a `difficulty` querystring on the server fetch + a client-side filter on the local view (verified: the request + board head change per difficulty); (b) removed the hard-coded NPC seed (VEX/KUNNING/RYO-9/NIGHT-OWL/ECH0) that rendered as real players in the same podium and confused testers — the dormant board is now an honest local-runs / empty-CTA view (the LIVE server board already shows real players); (c) fixed the share-card "unknown track" — `localRuns()` fell back to the truthy string `'Unknown track'` which poisoned every downstream `song||fallback`; now `''` (falsy) + `|| 'Untitled'` at render, so a leaderboard share carries the real title.
- **Review fix (MEDIUM):** the local Global board named every run `'YOU'`, and the podium renders only the name → it read "YOU / YOU / YOU". Now each entry is named by its track title (with difficulty in the sub-line) so your runs are distinguishable. (A latent difficulty stale-response race in the server fetch was noted + deferred to the leaderboard competitive-redesign batch.)
- Verified headless (rr-verify :8790, ?v=427): auth adopt path runs on a seeded session (getSession shows the session on fresh boot); the live Global leaderboard renders real players (no NPC names, no "YOU/YOU/YOU"); `adoptSiteSession` exposed; `node --check` clean. The big **leaderboard competitive redesign** (podium polish, movement arrows, you-vs-above, shareable rank cards) is a later visual batch.

### build114 — Playtest-4: authoritative MP chart (kills the "blank gaps between notes")  ✅ verified (?v=426)
The MP note-gaps root cause: MP had no shared chart — each client independently decoded + analyzed the audio, and browser audio decoders differ subtly, so the two players built DIFFERENT charts (one sparser → dead air). 2-agent adversarial review (2 HIGH + a pre-existing render crash → all fixed):
- **Host broadcasts one authoritative chart; guest consumes it.** New `RhythmGame.startAt(prov, {notes})` seam — when `notes` is supplied, `beginPlay()` skips its own `buildNotes()` and plays the injected array verbatim (snapshot-and-swap, one-shot, can't leak). New `chartUrlFull()` (a sibling of `chartUrlForSpectate` returning the FULL scoring-ready note shape, and force-clearing `_levelCtx` so a boss-stage MP pick doesn't get the sparser Hard easing). In `multiplayer.js resolveAndStart`, the host builds the chart once, broadcasts a compact ~15-36KB array on a new `'chart'` match-channel event, and launches off it; the guest polls a bounded window (≤3.5s within the lead-in) and injects the matching broadcast, falling back to a local chart if it doesn't arrive (never blocks the round). Scope: 1v1 + room matches only (tournament rounds have no live match channel — unchanged, flagged as a follow-up).
- **Review fixes (both verified):** *HIGH* — a RESTART/REPLAY during a live MP match bypassed the whole mechanism (the one-shot `_injectedNotes` was consumed, so bare `beginPlay()` re-charted locally → divergence returns). Added a non-consumed `_lastInjectedNotes` cache so a restart re-arms the SAME authoritative chart (no-op for solo/tournament). *HIGH* — the guest's `pollForChart` loop had no generation guard, so a zombie poll from a torn-down/rematched round could hijack a LATER round's chart. Added a monotonic `_resolveGen` token (captured per round, bumped in teardown/rematch) so a superseded poll bails.
- **Also fixed (pre-existing render crash, review-surfaced):** `drawCathedralBg`'s moon god-rays fed a non-finite value (`ch * energy`) into a gradient COORDINATE, throwing `createLinearGradient` non-finite EVERY frame whenever `energy`/`ch` went non-finite (observed headless; a real-play audio glitch would do the same and tank the frame). Added an `isFinite(len)` guard that skips the degenerate ray. (Not from build114 — a latent hot-loop crash; folded in since it directly serves the owner's "extremely stable / performance" ask.)
- Verified headless (rr-verify :8790, ?v=426): solo still charts 476 notes (notes-absent path byte-identical), injected `startAt({notes})` sets `notes.length` exactly + no leak into the next solo run, `chartUrlFull` round-trips all scoring fields, 0 console errors (drawCathedralBg guard confirmed); `node --check` clean. **True 2-client identical-chart convergence needs the owner's 2-device MP test.**

### build113 — Playtest-4: battle-call fixes (target guard + one-shot ring + self-heal)  ✅ verified (?v=424)
The "calling one person rings everyone / the sender hears it / the alarm gets stuck / the call won't go through" cluster — GAME side (the backend `notifications` RLS scoping is a parallel Lovable brief). 1-agent adversarial review (1 MEDIUM → fixed):
- **Target guard** — `_ringPoll` now checks the notification row's recipient id (all plausible field names, pending Lovable's schema confirmation) against `ME.id` and skips a row addressed to someone else. Closes both "rings everyone" and "the sender hears it" (the sender's own tab was polling the same mis-scoped row). Fails OPEN when no recipient field is present, so a legit call is never silently dropped while the field name is confirmed. Verified live: a row targeted at someone-else does NOT ring; a row targeted at ME rings once.
- **One-shot ring** — `_ringSeen` now also dedupes by room id, and `_ringAudio` is paused/cleared before any new `Audio()` (reentry safety) — so a retried `/challenge` can't stack a second card/sound.
- **Self-heal** — if `_ringUp` is stuck true but `#mpx-ring` is gone from the DOM, the next poll clears it instead of blocking all future rings forever; the menus observer fires an immediate poll on return-to-menus (was up to 30s); the invite window widened 2min→3min so a call sent while you were mid-song still surfaces when you get back. Verified live: external removal of the ring self-heals on the next tick.
- **Review fix (MEDIUM):** the room-dedupe was set permanently, so a legit SECOND call to the same room (a rematch, or a caller reusing a still-open private room to ring someone else) got silently swallowed for the whole session — itself a "call won't go through" contributor. Changed to a ~60s TTL: a rapid retry is still deduped, a later call is not.
- Verified headless (rr-verify-b :8791): target-guard skip, ring-once, self-heal, room-dedupe all confirmed via monkeypatched `notifRecent` + real poll ticks; 0 console errors; `node --check` clean. **True 2-client ring delivery (A calls only B, not A/everyone) needs the owner's 2-account test + the Lovable RLS fix.**

### build112 — Playtest-4 P0 bug batch: daily-card lie + win/lose scorecard + BEAT THAT  ✅ verified (?v=423)
First batch of the 3-tester playtest-4 haul (23 workstreams; roadmap in scratchpad/playtest4_roadmap.md, built from a 15-agent design/root-cause swarm). Three P0 fixes + a 2-agent adversarial review (4 confirmed → all fixed):
- **Daily-goal card lie** — the start-menu card showed "DAILY GOAL COMPLETE — see you tomorrow" while progress was 0/3 · 0/3. Root cause: `paintGoalsCard()` read `RG.state()` with no day-rollover, so it painted yesterday's `claimedDaily:true`. Fix: call `checkRollover()` before the card (and `showHub`) reads state. Also fixed a pre-existing CSS specificity bug (`.mgc-row{display:flex}` beat `[hidden]`) so the goal/complete rows actually toggle. Verified: seeded a stale prior-day state → after boot the card shows the checklist, not COMPLETE.
- **MP win/lose scorecard** — one player's verdict rendered broken while the other's was fine; results also jumped by too fast. Fix: rAF re-assert of the MP screen's `.active` (wins the last write regardless of arrival order) + a ~950ms reduce-motion-gated pacing HOLD before `showWinner()` (guarded by a `_settleGen` counter against the 8s-safety/rematch/teardown races). Added a **VIEW LAST RESULT** button to re-open the scorecard after the match.
- **BEAT THAT** did nothing / was undefined — it was hard-hidden in the standard verdict. Fix: wired it into the 1v1 loss/draw verdict (chase the winner's score), widened its gate from `audioUrl` to `trackId`, added tooltips, and scoped the watcher-only `endSpectate()` teardown to spectators.
- **Review fixes (all verified):** *HIGH* — `_lastVerdict` was stashed without a `!spectating` guard, so the new VIEW LAST RESULT would replay a fake "YOU LOSE" to a spectator who only watched (gated). *HIGH* — the Profile overlay's weekly-goal strip (`renderGoalsTrack`, reachable from the library not just the hub) read state without `checkRollover` — same stale-state bug on a second surface (fixed). *MEDIUM* — the new render-path `checkRollover` created a reentrancy: its weekly grace-payout `grantXp(200)` fires `rr-levelup` → repaints the card → re-enters `checkRollover` against unsaved state → **double-granted the +200 XP** (and a second level-up). Added a `_rolling` reentrancy guard — verified the rr-levelup listener re-enters but XP grants exactly once (300, not 1100). *LOW* — BEAT THAT's fallback called `playUrl(undefined)` for an uncached track → guarded + clearer banner.
- Verified headless (rr-verify :8790, ?v=423): daily rollover, reentrancy single-grant, 0 console errors; `node --check` clean. **The 2-client win/lose divergence + pacing need the owner's real 2-account test.**

### build111 — TEXT chat + moderation + reporting (client half; voice explicitly OUT of scope)  ✅ verified (?v=422)
Owner decree: text + moderation + reporting only — no voice/WebRTC/getUserMedia/TURN/SFU. Built per `voice_text_chat_plan.md`'s PHASE 1 §1A + Moderation sections, build-sequence steps 1–5, using the localStorage-buffered-report-evidence approach (§1B durable layer deferred). New shared `chat.js` (loads before catalog.js/multiplayer.js); `multiplayer.js` gains hook calls at its existing lifecycle points; zero new Realtime channels — everything rides the existing `lobbyCh` (`rr-lobby`) and room `ch` (`rr-room-<rid>`).
- **s1 — extract `LB_BADWORDS`:** new `chat.js` exposes `window.RhythmChat.filterChatText` (mask-not-reject, for chat messages) and `filterDisplayName` (whole-string mask, for names). `catalog.js`'s `scrubName` and `index.html`'s `cleanName` now delegate to `RhythmChat.filterDisplayName` when chat.js has loaded, falling back to their own local `LB_BADWORDS` copy otherwise — zero behavior change to existing leaderboard-name filtering.
- **s2 — lobby + room text chat:** new `chat-msg` broadcast event on the existing lobby/room channels. Send pipeline: trim → 140-char cap → `filterChatText` → client token-bucket rate limit (1 msg/1.5s, burst 3, over-limit = silent drop + toast). Receive re-runs the filter (defense in depth). In-memory ring buffer (last ~50), cleared on channel teardown so a rejoin never leaks a prior room's chat. UI (owned entirely by `chat.js`): lobby = collapsible bottom-right drawer with a crimson unread badge; room = ~240px scrollable panel mounted right after `#mpx-roomctx`, hidden the instant `room-start` fires. Message rows: avatar initial, name (gold if host), relative timestamp, text, report flag. a11y: list `role="log" aria-live="polite"`, input `aria-label`, reduce-motion → opacity fade not slide.
- **s3 — host mute/kick:** host-only kebab (⋮) next to the opponent name in the 1v1 face-off cell (this room model has no multi-row roster — that cell IS the row). `mod-kick` broadcast → targeted client runs the exact same leave path a manual "LEAVE ROOM" click already used (extracted to `leaveGuestRoom()`), shows a toast, sets a ~2min `sessionStorage` rejoin cooldown (`rr_kick_<rid>`) honored on `joinRoom()`. `mod-mute` (kind:`text`) hides that peer's chat bubbles for everyone honoring it. Both events verify `payload.byHost` matches the known room host id (`room.p1`) before honoring — blocks casual spoofing (not cryptographic enforcement, consistent with this codebase's existing client-trusted-scoring model). Added a 3-reports-in-a-session auto-soft-mute circuit breaker (session-scoped, client-side).
- **s4 — report flow (client half):** flag icon on every chat message, every lobby roster row, and the room opponent cell (both host and guest can report each other). Modal: reason chips (Harassment/Hate speech/Cheating/Spam/Other) + optional 140-char note (filtered). Reported user never notified — no broadcast sent. Session-scoped "already reported this target" guard. `RhythmCatalog.submitReport({targetId,targetName,roomId,matchId,reason,note})` — a stub that queues to `localStorage` (`rr_reports_queue`, capped 50) with the last ~30 buffered chat lines attached as evidence; structured so flipping to a live POST later is a one-liner. Fire-and-forget, silent-fail-safe.
- **Bug found + fixed during s4 testing:** `filterChatText`'s leet-speak fallback pass originally stripped ALL whitespace before substring-matching the wordlist — a "Scunthorpe problem" false positive (e.g. "was hitting notes" → "washittingnotes" contains "shit", masking an entirely clean sentence). Fixed to normalize+match per-word instead of across the whole message, and to mask only the offending word rather than the entire message on a fallback hit. Only touches the new `filterChatText` path — `filterDisplayName` (leaderboard names) is untouched, still byte-identical to the original `scrubName`/`cleanName` logic.
- **Verified** via `rr-verify-b` (:8791) with simulated Realtime channels (headless — no live Supabase connection in this environment): filter masks badwords/leet/spaced-leet and leaves clean text (including the false-positive regression case) untouched; a simulated inbound `chat-msg` renders a row in both lobby and room chat DOM; the 4th rapid send is silently dropped by the rate limiter while the first 3 go through; channel teardown clears the ring buffer and a rejoin starts empty (no leak); `mod-kick` with a spoofed `byHost` is ignored, a legit host kick targeting me sets the cooldown + fires the toast + calls the leave path, a kick targeting someone else is ignored; `mod-mute` hides/restores a peer's bubbles immediately; the 3-reports circuit breaker auto-mutes on the 3rd call; the report modal renders all 5 reason chips, submits to `rr_reports_queue` with reason/note/evidence populated correctly, and blocks a second report of the same target this session. `node --check` clean on every touched JS file; console clean after every step.
- **Deferred (explicitly, per scope):** §1B durable chat persistence (`rr_chat_messages` table) — ephemeral-only chat + the localStorage evidence trick instead. §1C voice entirely (owner decree). The `rr_reports` backend table/edge function (Phase 3 item 1) — being dispatched separately as a Lovable brief; `submitReport`'s stub shape already matches the future POST body.
- **Not yet real-world verified:** 2-client chat/mute/kick/report flows were only single-client-simulated headless (fake channel objects standing in for Realtime) — need a real 2-account test before this ships. Highest-risk areas for a reviewer: the `LB_BADWORDS` extraction (must not regress leaderboard filtering — verified via `filterDisplayName`/`scrubName` delegation + a same-output check, but worth a second look), the mute/kick host-id spoof check (`payload.byHost !== room.p1`), the rate-limiter's token-bucket math, and channel-teardown ring-buffer clearing (verified a rejoin into a fresh room starts empty, but only in the simulated-channel harness).
- **Review fixes (2-agent adversarial pass, 6 confirmed → all fixed at ?v=422):**
  - *HIGH* — the profanity filter **over-masked ordinary words**: pass 1 lacked word boundaries and pass 2 substring-matched within a token, so "cockatoo"→"***", "grapefruit"→"g***fruit", "shitake"→"***", "scunthorpe"→"s***horpe". Added `\b` to pass 1, and restricted pass 2 (the leet catcher) to tokens where normalization actually FOLDED leet — so plain words fall through to pass 1's boundary matcher. Verified: cockatoo/grapefruit/shitake/scunthorpe/assassin now pass clean while `shit`/`sh1t`/`sh1thead` still mask.
  - *HIGH* — **spaced-out evasion** ("s h i t") passed both filter passes; added a pass 3 that collapses a run of ≥3 single alnum chars and masks only if the collapsed run itself contains a badword (benign runs like "a b c d" / initials stay untouched — no cross-word Scunthorpe risk since it only joins single-char tokens).
  - *HIGH* — a sender's broadcast display **`name` was escaped but never profanity-filtered** on receive (a modified client could broadcast a slur as its name); both receive handlers now run it through `filterDisplayName`.
  - *HIGH* — the fixed **lobby chat drawer never hid when a match went live** → its "CHAT ▲" pill floated over the note highway (and overlapped the battle-call ring's ACCEPT) all song. Added `hideLobbyChat`/`showLobbyChat` (hide on `onRoomStart`, restore on `teardownMatch`, buffer intact).
  - *MEDIUM* — that pill sat at the same bottom-right corner as the battle-call ring (chat z-520 > ring z-340 → blocked ACCEPT); moved the drawer to bottom-LEFT.
  - *MEDIUM* — the host mute/kick **kebab menu was destroyed by routine presence-heartbeat repaints** mid-click (`paintModKebab`'s `innerHTML` rebuild wiped an open dropdown); it now skips the rebuild while its menu is open.
  - Re-verified live (rr-verify :8790, ?v=422): filter correctness across all cases, drawer bottom-left, `submitReport` queues correctly-shaped entries (cap 50), 0 console errors; `node --check` clean.

### build110 — Retention system: XP / Level / Streak / Daily+Weekly Goals  ✅ verified (?v=421)
New pure-client-side ledger per `retention-build-spec.md` (synthesized from 15 judged proposals — single build target, no competing schemas). Three new localStorage keys total: `rr_goals` (daily+weekly), `rr_streak` (flame+freeze), `rr_xp` (level ladder). All milestone unlocks are cosmetic-only, written into the **existing** `rr_bonus_owns` array — no new currency, no purchase path anywhere in the system.
- **Phase 0 — `goals.js` (new file):** pure ledger module, no DOM, no network. Exposes `window.RhythmGoals` (`state`/`xpState`/`streakState`/`goalsState`/`checkRollover`/`recordLevelComplete`/`recordMpComplete`/`claimDaily`/`claimWeekly`/`xpProgress`). Daily Goal = 3 levels + 3 MP matches (participation, not victory). Weekly = 15 levels + 10 MP (built-in rest-day slack). Streak resets to 1 (never 0) on a missed day unless a freeze is banked (auto-earned at day 7, every +7 after, cap 1, never purchasable — auto-consumed silently). XP curve: `floor(80*N^1.35)` cumulative (level 1 = free starting level, 0 XP required — fixed an off-by-one that otherwise permanently zeroed the level-1 progress bar). Loaded after catalog.js, before multiplayer.js.
- **Phase 1 — level-completion hook:** one guarded call inside `catalog.js recordLocal()`'s existing completed-run (`!failed`) block, beside the Bonus Sparks earn loop. Also added the results-screen **"+N XP" pill** (`#results-xp`, `renderXpLine()`) mirroring the existing Bonus Sparks line but kept visually/physically separate (own gold-pill styling) so status (XP) never blurs with currency-adjacent Bonus Sparks. game.js clears `#results-xp` each render alongside the existing `#results-bonus` clear.
- **Phase 2 — MP-completion hook:** one guarded call in `multiplayer.js recordMpResult()`. Win/loss/draw all count.
- **Phase 3 — celebration toast:** goals.js dispatches `rr-levelup`/`rr-streak-milestone`/`rr-streak-freeze-used` CustomEvents; index.html paints a new `#levelup-toast` (dynamically created, styled off `.rbadge.gradeup`'s gold/crimson language).
- **Phase 4 — "you're back today" hero card:** new `#mh-goalscard` in the start-menu hub — flame+streak, LEVELS/MULTIPLAYER TODAY progress dots, XP bar. Flips to a celebratory "DAILY GOAL COMPLETE" state once claimed (never re-nags a finished player). Dismissible per session only (`sessionStorage`, reappears fresh next boot).
- **Phase 5 — Profile XP + cosmetics:** new `.pn-lvlchip` ("LV 23") beside the existing rank label — a genuinely separate, faster-moving number; `rankTitle()`/`RANK_FLOORS` untouched. New XP bar under the existing rank bar, a "Total XP" stat cell, a "Recent XP" transparency feed. New `.profile-goals-wrap` **stacked block** (this profile screen has no tab system — confirmed by recon, so this was built as another vertical section, not a 3rd tab) holding the weekly-goal strip + the full milestone/cosmetic track, cloning `renderLoadout()`/`skinUnlocked()` verbatim for `namecolor:`/`notecolor:`/`noteskin:`/`flair:` items.
- **Verified** via `rr-verify-b` (:8791): full 3-level+3-MP daily-goal claim cycle, weekly grace-payout on stale-week rollover, streak freeze-consumed path, streak reset-to-1 path, next-day streak-continuity increment (via direct `lastCompleteDate` edit — **cross-day behavior is localStorage-simulated, not a real multi-day run; flag for a real multi-day check later**), locked-vs-unlocked cosmetic equip gating under `?asplayer=1` (fresh-player ownership path). `node --check` clean on every touched JS file; console clean throughout.
- **Review fixes (2-agent adversarial pass, 5 confirmed → all fixed at ?v=421):**
  - *HIGH* — the **weekly goal could not be claimed the same day it was hit**: `weekLevels`/`weekMp` only receive a day's total at the *next* day's rollover, so a player who cleared 15 levels + 10 MP in one session got no +200 XP until the following day (or the week-rollover grace payout). Now the weekly eligibility check adds today's live `levels`/`mp` (verified: one 15+10 session → `claimedWeekly` flips + xp 780 same-day).
  - *HIGH* — **local day-key paired with a UTC week-key** flipped the week early for US timezones: a UTC-7 player who played Sunday after ~5pm local (= Mon 00:00 UTC) had `checkRollover()` zero their weekly progress hours before their local week ended. `isoWeekKey()` now computes from LOCAL date fields (rr_goals.weekKey is independent of catalog's UTC rr_bonus_week).
  - *HIGH* — the profile screen called the two new render functions **un-try/caught inside `render()`, which runs from `open()` before `screen.classList.add('active')`** — any throw would silently break the *entire* Career/Profile overlay (icon click does nothing). Now each is isolated in its own try/catch.
  - *HIGH* — **brand violation**: a `rare_violet` name-color swatch `#b98ae0` (purple — forbidden). Recolored to on-brand crimson `#ff2e43` and renamed `rare_crimson` (in both the NAME_COLORS list and goals.js's XP-Level-12 unlock map).
  - *MEDIUM* — `load()`'s `typeof v === 'object'` guard accepted a bare JSON array (`typeof [] === 'object'`) as valid state, which would stringify back to `"[]"` and silently drop all named fields on the next save. Added an `!Array.isArray(v)` guard.

### build109 — VFX polish: winner/loser dopamine + marble notes + hit/OD juice + spectator mode  ✅ verified (?v=420)
Playtest-3's VFX/dopamine asks (the "YOU WIN should FEEL like a win," marble-note polish, spectator mode). Built as four serialized game-file commits, then a 3-agent adversarial review (5 confirmed findings → all 5 fixed before ship):
- **s1 — Winner/loser dopamine:** a generic `fireCelebrationOn`/`stopCelebrationOn` (sibling of the solo `celebrateResults`, own canvas + generation counter), exposed as `RhythmGame.fireCelebration`/`.stopCelebration`/`.shakeScreen`. A real MP win now fires the same confetti/firework burst + cheer/sting SFX + screen-punch + verdict scale-punch + one-time gold flash (reduce-motion gated). A loss under a 3% margin shows "SO CLOSE — -N pts" and pulses RUN IT BACK. Torn down in `resetForRematch`/`teardownMatch`.
- **s2 — Marble notes:** legibility rebalance + live specular rotation (a "rolling ball" highlight) + baked per-type overlays (accent/hold/chord, fxLite-gated) + closest-25% approach squash-stretch (reduce-motion + fxLite gated).
- **s3 — Hit/combo/OD juice:** ring particles now cubic-ease-out (impact deceleration, propagates to perfect-hit/combo-tier/OD rings); OD activation gets a ~140ms wind-up + rising stinger before the payoff; an additive-brightness cap so stacked OD-ember + combo-burst never blow out.
- **s4 — Spectator mode:** full 1v1 rooms stay discoverable + watchable in LIVE NOW (were dropped); the live match's `mid` rides the room-meta broadcast so a late joiner jumps straight into `spectateMatch()` instead of waiting on a broadcast that isn't replayed.
- **Review fixes (3-agent adversarial pass, all confirmed against the real code):**
  - *CRITICAL* — a WATCH spectator's `myFinal` defaults to `{score:0}` so they always compute a "loss"; the new s1 near-miss + RUN-IT-BACK-pulse block wasn't `!spectating`-gated, so a spectator watching a close match saw "SO CLOSE" + a pulsing rematch CTA for a match they never played. Gated the whole s1 block behind `!spectating` — restores the owner's decreed byte-identical pre-diff spectator card.
  - *HIGH* — `fireCelebrationOn` measured the target while the MP winner card was still `display:none` (it's unhidden a few lines later in the same synchronous `showWinner`), so `clientWidth/Height` were 0 → it sized the burst to the full window and rendered it squeezed/off-center into the small card. Now measures via `getBoundingClientRect` and defers one frame when the target isn't laid out yet (verified: canvas sizes to the element box, not the window).
  - *HIGH* — the marble specular `ctx.rotate` was applied unconditionally, so every falling note spun even with reduce-motion on. Gated behind `!reduceMotion`.
  - *MEDIUM* — ring-particle alpha used `Math.min(1,_t*1.4)` which pinned alpha to 0 for the final ~29% of life (invisible wasted strokes + an abrupt snap-out). Replaced with `sin(π·pow(_t,0.7))` — peaks early, fades to 0 exactly at end-of-life.
  - *MEDIUM* — a spectator who joined via a stale cached room `mid` (match already ended, channel torn down) could hang forever on a blank SPECTATING screen (the non-show path has no dead-feed watchdog). Added a ~14s liveness self-heal that bails to the room with "That match already ended."
- Verified headless (rr-verify :8790) at ?v=420: clean boot, 0 console errors, celebration API sizes to element box not window; `node --check` clean on game.js + multiplayer.js. **The winner-celebration FEEL + the 2-client spectator path need the owner's real 2-account pass** — headless throttles rAF and can't run two clients.

### build107/108 — playtest-3 round 2: roster consolidation + P1 fixes + battle-room hygiene  ✅ verified (?v=419)
Owner's live 2-tester session round 2. build107 (v418): the MP "PLAYERS ONLINE" list is now the single site-presence battle-call roster (CHALLENGE → /challenge site ring); the old lobby-presence roster + its broken lobby-broadcast challenge ("No response") retired; backend /presence/online filters to battle-calls-on users (admin sees all). build108 (v419) = the P1 batch + a 9-agent adversarial review (4 confirmed → all fixed):
- **Show-chrome leak fixed** (the big confusion): a normal/battle room was rendering the LIVE SHOW banner, ARTIST SEAT card, CALL IN THE ARTIST, TAKE THE STAGE, the ACCEPT/DECLINE challenger card, and BEAT THAT — all no-ops in a plain 1v1. paintShowRoom hid only two outer containers on !isShow; now it explicitly re-hides EVERY show-only control (mpx-show-invite/nudge/accept/decline/pend/swap/take*, mpx-beat-that/room), so a battle room is clean: just YOU vs them.
- **Endgame freeze-frame fixed** (the "broken UI before results"): stopGame() cancelled the render loop without resetting the FX state (particles/cameraShake/glitchAmount/bgPulse/missFlash/wipeoutT/stringsCold/odBurst), so the canvas froze on a garbled mid-effect frame that bled through the results crossfade. Now hard-resets that state + one calm final paint on teardown. Fail-out gets a real 550ms collapse beat (SIGNAL LOST animates) instead of a same-tick freeze — via a new `_endingLock` (freezes input+scoring, keeps FX decaying).
- **Review-fix (critical): pause during the fail-out beat stranded the run.** During the 550ms wipeout, an Escape/gamepad/blur/visibilitychange auto-pause flipped state off 'playing', so the deferred endGame() re-entry guard swallowed the only teardown → results never showed and Resume continued the "failed" run. Fixed: pauseGame() refuses while `_endingLock` (all pause paths route through it), and the deferred endGame only ends a still-live failed run.
- **Start mechanic**: countdown 10→3 (700ms/digit) + the guitar-materialize animation now completes BEFORE the 3·2·1 (notes no longer fall early); MP lead-in trimmed to 4.5s (shared atMs sync unchanged); TOUR lead-in 5s to match its picker (review fix — 4500 showed no active button).
- **Controller persistence**: traced + confirmed the wizard already saves + restores padMap/strumCfg/hit-mode across reopen.
- **MP UX**: score-HUD text-bleed clamp; **cover thumbnails on the track picker AND the stage chips**; Browse/Host surfaced in the lobby; TAKE STAGE tooltip; a real BACK-TO-LOBBY pill on the verdict.
- **Anti-mash**: empty presses already score/combo-neutral (confirmed P1+P2); added a display-only sloppiness tally in results (never touches score/grade/leaderboard). Review-fix: results sloppy + auto-calibrate notes now anchor in a stable order.
- **Alarm-won't-stop (site, committed)**: BattleCallAlert hard-stops the ring on ACCEPT (defeating a play()-promise race) + marks the notification read so neither site nor game re-rings.
- Verified headless (rr-verify :8790): clean boot at ?v=419, 0 console errors; `node --check` clean. **Owner 2-account pass owed** on the battle-room-clean flow, the fixed end-transition, and the ready→auto-start.

### build106 — MP HANDOFF ROOT-CAUSE FIXES (playtest-3, live 2-tester failures)  ✅ boot-verified (?v=417)
Owner tested MP with a second person; the matchmaking/room handoff broke in real 2-client play (headless + code review had passed — the lesson: never call a 2-client flow done without a 2-account test). A 4-investigator root-cause swarm (wf_d045fe24) traced it to **4 client-side state-machine bugs**, all in multiplayer.js — no backend change needed. Fixed all 4 + the owner's ready/start redesign:
- **Bug A — host battle-call ACCEPT stranded the caller.** The site's ACCEPT deep-link is `?mproom=<rid>&mprole=host` (no `?mpqm=resume`), so `_pendingQmResume` was false → `qmResume()`/`_qmHostFallback()` never ran → `_qmBootRoom` was a dead-letter box → the accepting HOST sat on the hub while the guest landed in the room. Fix: the lobby-SUBSCRIBED handler now consumes `_qmBootRoom` (`else if (_qmBootRoom) _qmHostFallback()`), and the boot auto-open raises MP for the host too.
- **Bug B — deep-link room join was a one-shot 6s ping, no retry.** A slightly-slow host (cold Realtime / backgrounded tab) missed the single ping → "room isn't open anymore" even though the meta was about to arrive → users "had to back out and make a fresh room." Fix: the `_pendingRoomJoin` join now uses the proven `setInterval(2.5s)×12 ≈ 30s` retry (mirrors the qmMatched-guest loop), clearing the instant `onRoomMeta` auto-joins.
- **Bug C — stale `rr_showroom` resurrected show-seat chrome on a NORMAL room.** An abandoned GO-LIVE/review test left the key; a later boot's `maybeReconnectShowRoom()` restored `room.show=true`, which advertised to the guest via room-meta → a plain duel showed "accept the challenge" + "he is being called in" + a stranded TAKE STAGE / challenge-seat. Fix: `clearShowRoom()` at the top of `openRoomWithId()` and on normal `joinRoom()` — a normal-room action kills the stale key so it can't resurrect.
- **Bug D — no room-stage ready/start protocol → "both in the room, couldn't start."** `refreshReadyEnabled`/`toggleReady`/`maybeStart` all read `oppPresent`/`matchCh` (match-channel-only), but the ready step happens on the ROOM channel where neither exists yet → READY was disabled or its broadcast went nowhere → the start handshake deadlocked for a normal duel. Fix + **owner's ready/start redesign** (each readies independently → auto-start when both green): a new `'room-ready'` broadcast+listener on the room channel + a room-scoped `roomOppReady`; `refreshReadyEnabled` is stage-aware (host waits for `room.p2`, guest enables once a track is picked; re-evaluated on every `paintRoomWaiting`); `toggleReady` broadcasts on the room channel in the room stage; the host **auto-fires `startRoomMatch` the moment both are ready** (no separate Start button); a track (re)pick un-readies both sides. Show rooms are explicitly excluded (`!room.show`) — they keep the host-starts-alone behavior.
- Verified headless (rr-verify :8790) at ?v=417: clean boot, MP opens, READY present, 0 console errors; `node --check` clean. **The 2-peer handshake + the visible green-checks/auto-start FEEL need the owner's 2-account pass** — headless can't run two clients.

### build103/104/105 + review fixes — VISUAL EVOLUTION + GAMEPLAY (CHART_VERSION 2) + ONLINE-NOW battle-calls  ✅ verified (?v=416)
The owner's post-playtest overnight batch. Two design swarms (gameplay: 4 critics × 3 judges over 30 findings; visual: 3 lanes × 3 judges over 15 proposals) produced build-ready plans; two builders executed them in parallel (game.js vs index/multiplayer/procbg) — BOTH ran out of credits mid-verification, so the coordinator inventoried the tree (clean, boots), checkpointed both works, closed the one unreached step (CHART_VERSION), then ran a 14-agent adversarial review (8 distinct issues → 7 fixed; 1 "critical" P2-chord-bonus finding was re-verified by the coordinator as a FALSE POSITIVE — P2 pays 0.3× per member vs P1's 0.3× per chord-total, mathematically equal, so the scorer was left untouched).
- **build103 — VISUAL EVOLUTION (owner: MP UI "wall of text"; VS bar; Tier-I backdrops):** the MPX design system (`--mx` spacing tokens, `.mxc` card tiers ACT-crimson/INFO-chrome/GOLD-earned, `.mxh` heads, one-crimson-per-step law, emoji→SVG sweep, `mxIn` stagger) applied across all MP screens (winner = one hero action not seven buttons, rooms, lobby, LIVE-SHOW card) — every #mpx-* id + hidden-wiring preserved. **VS TUG-OF-WAR bar**: center-origin spring physics (chrome diamond knot, endgame heat, lead-flip spark canvas, fxLite/reduce-motion gated). **Tier-I backdrops**: First Light (a sunrise that physically RISES with combo — pixel-probed luminance 16→195), Steady Hands (bottom-up string ignition + blade sweeps), Ember Drift (VP-outflow ember firestorm) + a shared camera rig (breathe/kick/OD-zoom, vertigo, reduce-motion→identity) + earned combo cutaway.
- **build104 — GAMEPLAY, CHART_VERSION 2 (owner: "more responsive, better gameplay, more intelligence to the charting"):** s1-3 runtime — late-edge miss race fixed (miss declared at time+hitWindow+0.06, hit legality unchanged), audio-latency seed (outputLatency), results-screen auto-calibration chip, press-mode gamepad fast poll. s4-10 chart-side (one leaderboard epoch) — grid phase refinement + bounded snap (never crosses the Perfect window), post-insert sanitizer (ping-pong reflection not modulo-wrap, maxJump clamp, time-aware jack→trill cap), hold overhaul (same-lane clearance, sustains to 3.5s, **duration-scaled payout** 140+110·min(1,tail/1.2), graded early-release bands 0.45/0.75), unified chord + FULL CHORD 1.3× bonus, accents = 1.5× money notes + budgeted stars, pattern vocabulary (trill/stair alternation, chart-relative gates, centroid-following, Medium 4% budget), finale/ramp-in/chord guarantees. s11 — the real **`CHART_VERSION = 2` constant**, stamped on every `/score` leaderboard submission (v2 charts+scoring ≠ v1, so the backend can segment eras) + mirrored in the P2 shadow scorer + the score-ceiling math. **Leaderboard note: v2 opens a new scoring era** — see Lovable follow-up.
- **build105 — ONLINE NOW roster + direct battle-calls (owner: "pick and choose any people on the website to battle-call"):** `RhythmCatalog.presenceOnline()/challenge()/notifRecent()`; a lobby ONLINE-NOW card lists everyone on the site+in-game (GET /presence/online, <90s), each row a **BATTLE CALL** that opens a private room + POSTs /challenge → the target's SITE rings (their ACCEPT deep-links into the room); game-side /presence/ping heartbeat (60s, pauses on hidden); an in-game battle-call ring card (own-rows notifRecent poll, menus-only, loops battle-call.mp3). Owner (admin) bypasses the target's toggle. Backend + Navbar two-toggle split (Auto-match / Battle-calls) shipped by Lovable.
- **Review fixes (7):** (1) the in-game ring now DIES on game start (MutationObserver on #game/#loading — was persisting with looping audio over live play); (2) a video-review is flix-launched ONLY when a real non-HLS video backdrop exists, else forced media_type:'audio' (a track merely TITLED "…Official Video" no longer gets a black backdrop / dead button); (3) BATTLE CALL always routes a PRIVATE room (never hijacks an open public one) + (4) tears down the room it opened if /challenge fails (no orphaned advertised room); (5) ONLINE-NOW poll cleared on any MP-screen deactivate; (6) #mpx-ring z 320→340 (was tied with the couch overlay); (7) procbg reduce-motion cutaway fully inert (st.hold gated like st.cut — no freeze/gold-burst).
- **Also (build102z p1):** local-versus popup z 120→320 (was hidden under the MP screen), battle-call ACCEPT boot fallback (?mproom+?mprole lands the host IN the paired room when the pairing record expired), signed-in identity re-resolve on MP activation (killed the false "Sign in"), back-to-website pill on the menu hub.
- Verified headless (rr-verify :8790) at ?v=416: clean boot, 0 console errors, CHART_VERSION=2 exposed, roster APIs live, CHART_VERSION-2 demo chart builds healthy (465 notes / 66 holds / 68 chords / 3 bombs). `node --check` clean ×5. **Owner test pass owed:** the VS-bar feel + backdrops on a real 60fps machine, and the battle-call/roster ring end-to-end with 2 accounts.

### build102z p1.5 — VIDEO submissions play flix-style through "Play in Reactive Rhythm"  ✅ verified (?v=414)
Owner mandate: a video/movie submission must still be playable from the live panel's button — the video IS the level, full-bleed behind the highway, guitar as normal.
- **Detection (`_rvIsVideoUrl` + `isVideo(r)` on the resolve payload):** same fuzzy class as the `mediaType()` seam (no backend media_type on `/review/resolve` either) — a video-container extension (`.mp4/.m4v/.mov/.webm`) on the chosen playable URL, or the payload's own genre/title hints. An .mp4's audio track decodes in WebAudio, so chart timing = video timing (the exact AI-Flix contract). Exposed as `__rrReview.vid` (dev hook, strip at launch).
- **Solo PLAY:** a detected video review routes through `playFlix` (backdrop staged on `#bg-video`, `flix-mode`, clearEnvironment, currentTrack set → scoring/leaderboard identical to an audio review). Backdrop = the charted media itself when it's a video file (same file → frame-perfect sync), else the payload's best non-HLS video field; an HLS-only film still plays off its decodable audio (dark backdrop beats the old behavior — `launchTrack` hard-refuses `isVideo` tracks, so a video-classified review PLAY was a dead button).
- **Launch card:** the STAGE chip strip hides for a video review (the video IS the stage); difficulty row + title/art unchanged.
- **GO LIVE (v1 scope, honest):** the backdrop URL rides `sel.flixVideo` — host + seated challenger both stage it in `beginMatch` (each fetches the video; sync = each seat's own engine clock, starts are already atMs-synced). SPECTATOR decks stay video-less: `#game:not(.active) #bg-video{display:none}` — a spectator never activates the game screen, so the element physically can't render there without new machinery. Audio charting for every watcher is unchanged (sel.audioUrl decodes the mp4's audio fine).
- **Cleanup:** the flix rAF self-tears at run end; `teardownMatch` backstops it (gated); `sel.flixVideo` dies at every `delete sel.audioUrl` review-fix-1 site; `_reshowReviewCard` keeps the flix identity on the solo-fallback card.
- Verified headless (rr-verify :8790, muted, ?mock=1): 14/14 URL classifications, video card hides the stage strip (diff row live), PLAY → `flix-mode` + `#bg-video`=the submission's video + 337-note chart, exit → class off + moon-loop restored; audio review card byte-identical (13 stage chips, launch w/o flix-mode); 0 console errors; `node --check` clean ×2.

### build102y — the MP ECOSYSTEM push: stage picker + OPEN THE ROOM (owner mandates) + 4 competitive-loop wins  ✅ verified (?v=412)
Owner's overnight mandate ("faster better ecosystem of the multiplayer through the game and the platform… foster cooperative and competitive"). A 9-agent judged design workflow (5 lens designers × 3 judges over 20+ proposals) produced the build plan; one builder shipped 6 features in 6 commits (v406–411); an 18-agent adversarial review confirmed 13 findings → 8 distinct fixes (v412). v406–411 were never published; v412 supersedes.
- **STAGE PICKER on the review launch card (owner ask):** the host picks the LEVEL for a review run — chip strip of ARENA + every owned finished level (`_rvStageList`, entId slug-gotcha honored). Solo PLAY = `applyEnvironment` + `launchTrack({keepEnvironment:true})`; GO LIVE carries `env` → `sel.env` — the show snap ships `sel` wholesale so the challenger + every late-joining spectator get the same stage free. Edge cases shipped: paid chips hidden when the card can GO LIVE, card re-open resets to ARENA + `clearEnvironment`, the HLS watch-fallback hides the strip.
- **OPEN THE ROOM (owner ask):** 3-way seat policy under GO LIVE — **ARTIST ONLY** (default, byte-identical), **OPEN SEAT — I CONFIRM** (anyone can request; host accepts via the existing pending card; submitter still jumps the queue), **OPEN SEAT — FIRST COME** (auto-seats via `acceptChallenger`, gated `!room.p2 && !matchLive`; built but DARK behind ?dev=1 until a real show soaks). **TAKE THE STAGE / WITHDRAW** for spectators (presence-seat flip), **SWAP IN THE ARTIST** gold pill (demote-by-snap, mid-run safe per fix A), host-authoritative **pendIds/pendN** queue on the snap (kills two-watchers-both-see-#1), `openSeat` persisted across host refresh. **LIVE NOW billboard**: show rooms now advertise cover + "Song — Artist" + stage + watcher count + OPEN SEAT tag with WATCH / **TAKE THE SEAT** buttons (`esc()`/`safeUrl()` on every peer string); `livematchAnnounce` sends `{title, open}` (both call sites) so the site chip reads "LIVE: Song — seat open".
- **NEXT RIVAL** on the verdict screen: one tap = leave the finished room (fix F: FULLY exits — a still-joined room made you invisible to every other matchmaker via the `!p.room` filter) + instant requeue; hidden on show/spectator verdicts.
- **SHARE THE W / RUN IT BACK + COPY BATTLE LINK** on the verdict: branded battle card (share.js versus caption — winner brags, loser summons a rematch; fix G: real album art + real difficulty on the card), battle link = the ?mproom invite (never on priv/qm/show rooms); share fires inside the click (transient activation).
- **RIVALRY LEDGER** (`rr_mp_rivals`, local-only, LRU-50): lifetime head-to-head per opponent via `recordMpResult(oppId)`; roster rows wear a crimson **REVENGE** chip + "you trail W–L" when you're down; the verdict prints the lifetime flip. Cosmetic-only (leaderboard-safe).
- **BEAT THAT** ghost duels: at both spectator exits (spec verdict + waiting card), one tap launches the SAME track chasing the winner's score — display-only gold target pill + "BEAT IT! 🏆" flash in the engine (zero scoring interaction), RETURN TO THE SHOW on the results screen (painted only while the show room is alive). The spectator→player funnel, and it drives plays on the artist's actual track.
- **Review fixes (8, from 13 confirmed findings):** (A) never demote an ACTIVELY-PLAYING challenger on a SWAP snap (was silently dropping their settle + ladder record); (B) WITHDRAW-vs-ACCEPT race wedged the seat on a self-declared spectator — host now vacates on presence truth with a 10s freshness grace (`room._p2At`); (C) `_ghostRunActive` parks room-start/auto-spectate during a ghost run (the host starting the next run mounted the dual-deck stage + a SECOND audio stream over live gameplay) — cleared on results paint/RETURN/MP-open/closeRoom; (D) ghost target now dies in `stopGame()` (the pause-EXIT path leaked a stale pill + BEAT IT! flash into later MP/demo/show runs); (E) the spectator verdict hides the four player-verdict controls (stale SHARE THE W shared the previous duel); (F/G above); (H) peer-supplied final scores clamped finite (`{score:'1e999'}` → Infinity rendered '∞' and an unwinnable ghost) at `setGhostTarget` + both `_lastShowFinal` build sites.
- Verified headless (rr-verify :8790): boot clean at ?v=412, 0 console errors, Infinity ghost rejected + null clears the pill, all 8 new UI elements present; per-step evidence in the build run (13 stage chips live, real show room opened over live Realtime with `openSeat:'confirm'`, full BEAT THAT loop incl. the flash in a live muted run). `node --check` clean ×3. **Owner 2-browser pass still needed:** step-5 seat races (withdraw-vs-accept, host refresh mid-queue), GO LIVE with a stage picked (both seats + late spectator same backdrop), rivalry/NEXT RIVAL/share in a real 2-account duel, BEAT THAT off a real show.

### build102v/w/x — wizard self-config + strum feel + SITE-WIDE MATCHMAKING (Phase 3)  ✅ verified (?v=405)
Three increments (one builder agent) + a 15-agent adversarial review (9 confirmed findings → 6 distinct fixes applied). v403/404 were never published; v405 supersedes.
**build102v — controller wizard declares the mode (owner: "review the setup controller function"):**
- The wizard now **self-configures HIT MODE**: learning a strum in the wizard → Strum-to-hit ON; skipping the strum step → Press-to-hit. `applyGhPreset` (GH auto-detect) also arms Strum. A `rr-strummode` CustomEvent keeps the Settings row in sync; press-mode pads now fire Overdrive from the wizard-captured `spBtn` too (was odBtn-only). Dead `set-lanemode` wiring removed (game is 5-LANE by decree); Input-Test pips 6→5.
**build102w — strum FEEL (owner: "push hard to register", holds "weirdly tiring"):**
- **70ms strum-before-fret buffer** (`_strumBufT`): the classic GH stroke order (bar a hair before the fret) now banks and pays on the fret press instead of eating the stroke. STRUM_DEBOUNCE 55→35ms. A **~4ms fast input poll** runs during strum-mode play only (rAF-queue delay read as "needs a hard push"; self-disarms on pause/end/mode-off). **Hold inertness**: a lane mid-sustain is inert to strum edges — strumming other lanes no longer re-judges the sustaining lane (ate next notes / hit bombs / dropped the sustain / whiffed every stroke — the "tiring" hold feel). Guitar-source empty strums are free (no whiff cue). Dev dials: `__rrStrumFeel.get()/.set({bufMs,debounceMs,pollMs})`, `__rrPollPad` (strip-listed).
- **Review fixes:** (1) the fast poll passes `liteAxes` and skips the **whammy** block — the whammy→OD charge is a per-sample delta gate, and 4ms sampling made a casual wiggle charge NOTHING (and a fast wiggle ~4x + 250Hz HUD writes); whammy stays rAF-sampled. (2) a strum where EVERY held fret was sustaining was swallowed AND un-banked — now it banks like an empty strum, so strum-then-fret works during holds (the exact complaint this increment targeted).
**build102x — matchmaking client + live-match site chip (PHASE3_MATCHMAKING_DESIGN.md):**
- catalog.js: `matchQueue(on)`/`matchStatus()` authed wrappers (throw on !ok — 401 vs transient distinguishable); `livematchAnnounce/livematchEnd` (review-token, module-local, fire-and-forget) called from the show-room SUBSCRIBED host branch + closeRoom's show branch → the site's LIVE MATCH chip. **Chip keepalive**: the backend expires an unrefreshed live_match after ~5min, so the host re-announces every 30th show-snap tick (=120s).
- multiplayer.js: `openRoom` core extracted → `openRoomWithId(rid, opts)` (normal create-room path byte-identical); `RhythmMP.queueMatch(on)` — POST {on:true}, 4s status poll + 30s heartbeat, matched: host opens the server-minted `qm########` room (private), guest rides the ?mproom auto-join with a 2.5s re-ping loop; lobby pill `#mpx-qm-pill` (OFF/SEARCHING pulse/MATCHED); `?mpqm=resume` deep link (the site's MATCH FOUND CTA) with one-shot `qmResume()`; teardown on toggle-off/leaveAll/screen-deactivate; 401 → sign-in banner, 404 → honest "isn't live yet" stop (kept as a deploy-regression guard — the routes went LIVE 2026-07-02 after an edge-fn redeploy).
- **Review fixes:** (3) entering ANY room (host or join) cancels a live search, `qmMatched` refuses while `room.id/tour.id/matchLive` (a match landing mid-room stomped the live room and orphaned the seated friend — no room-gone ever sent), and the guest retry loop clears its stale `_pendingRoomJoin` when bailing (was a landmine: a later re-advertise auto-joined the player into a dead qm room without consent); `queueMatch(true)` refuses from inside a room. (4) `_pendingRoomSpec` (?spectate=1) is now a true one-shot — a leftover flag made a matched GUEST auto-join their own match as a SPECTATOR (host waits forever). (5) the '🎯 MATCHED!' pill un-sticks on give-up + room close. (6) `qmResume` gets a 4000ms auth-hydration grace (same as /review/resolve) — a slow session restore 401'd the site's MATCH FOUND handoff and told a signed-in player to "sign in".
- **Backend + site (Lovable, all LIVE):** rr_match_queue + POST /match/queue + GET /match/status + /livematch/announce|/end (first deploy predated the routes — probed 404, one redeploy fixed; all four now 401-for-anon); /livematch/announce accepts {title, open} + 5-min TTL; Navbar MatchmakingToggle.tsx; BattleCallAlert.tsx (loops assets/battle-call.mp3 on a challenge until accept/decline, 45s missed-call, 5-min decline dedupe); sync-game.mjs now mirrors assets/ into public/game/assets/ (battle-call.mp3 was hand-placed before this).
- Verified headless (rr-verify :8790): boot clean 0 errors at ?v=405, all 5 new APIs live, pill SEARCHING → real 401 from the LIVE route → sign-in banner → pill reset. `node --check` clean ×3.
...
### build99j — local split-screen co-op (P1 keyboard vs P2 controller) (?v 344→345, game.js + couch.js)
Owner-greenlit. Makes the build79 couch claim/verdict groundwork a REAL local versus: two highways on one screen, same song, independent scores, top score wins. Built as an **additive "shadow scorer," NOT an engine clone** — single-player and online MP stay byte-identical when couch mode is off.
- **`RhythmGame.createEngine(opts)` (game.js, additive, default-inert):** the contract `couch.js` already sketched. Spins up a SECOND, PARALLEL P2 scoring layer that READS the shared authoritative state (`notes[]`, `songTime()`, the active `DIFFICULTY` hit window, `JUDGE`, the exact `curMult()` formula) and keeps ALL of its own state in a separate `_p2*` namespace: `score/combo/maxCombo/counts`, a `judged` **Set of note indices** (parallel to P1's `notes[].judged` — never touched), `frets` (P2's held-fret set, separate from P1's `_frets`), `padPrev` (separate edge state), `holdNote/holdScored/laneDown` (P2 sustains), and a drained `ev` buffer. It **never** reads or writes P1's `_frets`, `_padPrev`, `score`, `combo`, `counts`, or the shared `notes[].judged/hit` fields.
...
- **P2 deck render (couch.js, ported from `multiplayer.js renderGhost`):** a second highway on `#couch-hwy2` (LEFT cell) drawn from the public `RhythmGame.getLaneFrame()` + `getGuitarArt()` + P2's OWN `getP2Ghost()` (the SAME falling chart, with gems FADING as P2 hits them) + the drained `getP2Frame().ev` (per-lane catcher flash white-hot on a hit / crimson on a miss + sparkles). Strings, perspective, neck-recede warp and lane colors all mirror P1's deck. Reuses the existing **vs-mode split-screen layout** (`#game.vs-mode` two-column grid + `setVsMode(true)` + the `vs-opp-deck`/`vs-seam` CSS) so the polished chrome applies for free; a center seam shows the live P1↔P2 score delta.
- **Flow:** `RhythmCouch.startMatch()` (already wired to the MP "Local versus" tile + `?couch=1`) now mounts the deck, calls `createEngine({ canvasId:'hwy2', expose:'RhythmGameP2', isP2:true, gamepadIndex:P2.index, onSongEnd })`, starts P1's song (the bundled **demo** track — reliable + fully local; a `getCouchTrack` seam is in place for a future "play the picked track" path), and on song end calls `RhythmCouch.showVerdict(p1Score, p2Score)` → the gold WIN/LOSE/DRAW screen. The verdict overlay is now **body-parented + deferred one macrotask** so the engine's `showScreen('results')` can't hide it. Escape/Cancel/Done tear down the P2 layer + exit vs-mode cleanly.
- **Additive-only proof (verified, rr-verify 8790):** with couch OFF, `playDemo()` scores normally, `#hwy2`/`#couch-hwy2` stay hidden, `#game` has no `vs-mode` class, `RhythmGame.hasP2()===false`, zero new console errors. **Couch path (verified headless):** `RhythmCouch.open()`→`_claimP1()`→`_claimP2(0)`→Start runs `createEngine` without crashing, split-screen engages, the second deck renders the chart, and with a mocked `navigator.getGamepads` injecting P2 presses `_p2state.score` increments independently of P1's. `node --check` clean on game.js / couch.js / multiplayer.js.
...
### build99L — systems polish (UI-only; gameplay byte-identical) (?v 345→346)
Near-launch UI/UX polish across the start screen, menu-hub, career/profile, leaderboard, settings, results, and store surfaces. Render/markup/CSS + render-only JS DOM builders ONLY — the gameplay/scoring/input engine, the couch P2 shadow-scorer, and the MP netcode were not touched. Brand guardrails held (warm darks R>=G>=B, crimson/gold accents, no purple/blue).
- **Start screen + Menu-hub:** the START pill now fades up at ~1.0s (right after the 0.85s wordmark slam) + pulses from 1.6s — was 1.95s/2s, which read as "still loading" for ~3 dead seconds. **Campaign** is now a full-width **hero banner** (`grid-column:1/-1`, horizontal layout, 30px title, a "NEW · START HERE" / "CONTINUE" chip) so the intended primary dominates; the other five tiles flow into a secondary 3-col row beneath. Added a slim **"me" status strip** (avatar · career rank · Bonus balance · CONTINUE: <last level>) under the hub title, populated render-only by `paintHubStatus()` from existing `RhythmCatalog`/`RhythmLibrary` getters (the profile's `rankTitle` ladder is now shared via `window.__rrRankTitle`, no duplicated thresholds). Added a feathered dark **scrim** above the hub video (and dropped video opacity 0.42→0.34) mirroring `#menu .lib-bg-scrim` so tiles never float on raw video. Promoted **"Skip to library"** to a ghost-button (pill/border/hover glow) renamed **"Browse all tracks →"**. Verified live on rr-verify: hero spans 1/-1, chip flips to CONTINUE for returning players, status strip shows rank "RIFT TAPPER" + Bonus, scrim present, 0 console errors.
- **Career / Profile (`#profile-screen`, render-only):** lifetime stats now **count up** 0→target on open (~700ms ease-out, ~60ms stagger, `toLocaleString` per frame) — `html.rr-reduce-motion` sets finals instantly. **Lifetime Score** is now the trophy: a span-2 first-position cell with a gold/crimson frame + a 34px number (the other KPIs stay 26px). Added a **gold rank progress bar** under the rank label, filled to (runs−floor)/(next−floor) on the runs ladder (0/5/15/30/60/120; S≥25 = MAX RANK), caption "RIFT TAPPER · 12/15 to ECHO RUNNER". Converted the **grade distribution** from five flat equal count-boxes to **horizontal bars** (width = count/maxCount on the S=gold / A=coral / B/C/D ladder, count right-aligned; S gets a bigger letter + inset gold glow), widths grow on open. Off-brand salmon A-grade shifted to warm coral #e07a4a (R≥G≥B). Reveal uses setTimeout (not idle-rAF) so it's reliable even when rAF is throttled. Verified live: count-up lands on correct values, bars fill (S 33% / A 100% / B 67% / C 33%), rank bar 70% at 12 runs, reduce-motion instant, 0 console errors.
- **Leaderboard (`#leaderboard-screen`, render-only):** the default **Global** tab now renders a **seeded aspirational ladder** (VEX/KUNNING/RYO-9/NIGHT-OWL/ECH0 with grade badges, a "preview — beat these" banner) with the local player's best score ranked in — or a pinned "#— YOU · play a song to claim your spot" row when there's no run yet — instead of a dead empty board (the local fallback path; the live backend still wins when it returns rows). Raised `.lbd-board` min-height 60→240px and gave `.lbd-empty` a **centered state** (icon + text + a crimson **PLAY A SONG** CTA that opens the jukebox) so an empty board no longer collapses to a void. **Podium 1st now towers**: taller pedestal (translateY(-14px)), a 👑 crown above the avatar, and a slow gold pulse (reduce-motion/fx-lite gated). The per-song tab now **defaults to the most-recently-played track** (RhythmGame.lastResults → highest local score) so it opens populated. Verified live: seed ladder + crown + −14px pedestal render, empty CTA opens the library, song tab auto-picks the recent track, 0 console errors.
- **Settings (`#settings-screen`):** grouped the flat ~18-row wall into four labeled **AUDIO / GAMEPLAY / VISUALS / CONTROLS** sections, each a crimson Chakra-Petch eyebrow over a hairline divider, so it's scannable (pure markup grouping — every control id + JS wiring unchanged; Note Speed moved under GAMEPLAY, volumes under AUDIO, FX/motion/background under VISUALS). The controller-**wizard's bottom button** now reads **"CANCEL & CLOSE"** during the fret-mapping phase (was "CLOSE", which looked like SAVE but actually cancels & restores the prior map) — done render-only via a MutationObserver on the wizard's visibility + `window.__rrPadWizard()` phase check; game.js still owns the "DONE" label at the calibration/finish phase. Verified live: four section headers render crimson, all controls present, wizard button shows "CANCEL & CLOSE" mid-capture, 0 console errors.
- **Results (`#results`, markup + render-only):** the 5-button wall (three competing heroes) is now **ONE clear primary per context** — level active → NEXT LEVEL is the single crimson primary and PLAY AGAIN demotes to a ghost (via `#results.level-active`, toggled in `wireResultsLoop.refresh()`); free-play → PLAY AGAIN is primary and NEXT LEVEL is hidden. SHARE stays the secondary gold; CAREER + RETURN-TO-MENU moved to a quieter secondary row. **Reserved gold for achievement**: NEXT LEVEL is now crimson-primary (was a gold button) — bright gold stays for S / NEW BEST / earned badges. Added `max-height:92vh; overflow-y:auto` to `.results-wrap` so the 220px grade stamp + summary + breakdown + buttons no longer clip on 13" screens (mirrors `.howto-card`). **Final Score KPI** bumped to 40px (other KPIs stay 28px) + a gold accent on a NEW BEST (`#results.is-newbest`, toggled render-only in `pulseShareIfBrag`). Verified live: context primary swap works both ways, SHARE gold, Final Score 40px gold on new-best, wrap scrolls, `node --check game.js` clean, 0 console errors.
- **Store (`#store-screen`, render-only):** the guest CTA is no longer split/self-defeating — for a signed-out shopper the Sparks button is now the clear crimson **"Sign in to buy"** primary (was a bare "Sign in" that dead-ended next to a competing "Use Bonus"), and **"Use Bonus"** demotes to a small gold underlined secondary text-link. The grid now **sorts featured → new → by rarity** (legendary/epic first) before mapping, so the store opens on hero merchandise (stable within each tier). Verified live: sort runs clean and leads with featured/high-rarity items; `.signin-primary` resolves to the crimson primary + `.bonus-secondary` to a 10px gold underlined link, 0 console errors. (Guest buttons can't render on the localhost dev build since DEV_UNLOCK_ALL owns everything — verified via the equivalent CSS resolution + code path.)

### build99M — Launch prep: public-free confirmed + FPS-safety + support channel (?v 352→353, game.js + index.html)
Owner decision: **public free-to-play release, no beta codes.**
- **Beta gate** — verified ALREADY public (build66 disables it: hidden immediately on load, no overlay flash, no /beta/status dependency). No change needed; the owner's "public free beta" matches the live code.
