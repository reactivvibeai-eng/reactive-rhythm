...
- **NEXT (fresh context):** (1) **implement Phase 4** (task #163, CAREFUL — gameplay/input: independent CPU rival so you can LOSE, GH overstrum + chord fret-settle behind a Strict-Strum toggle default OFF, KEYBOARD BYTE-IDENTICAL — spec in w1bxkg5rd; a vetted impl plan is in flight); (2) apply the **24 design-evolution ideas** DO-NOW shortlist (QA2_FINDINGS.md §B); (3) **Phase 6** (task #165, XL — AI Flixs playable video-bg level [blocked on decodable per-video audio_url] + Local Versus ?couch=1, 2-person HW test); (4) keep cycling critique/evolution rounds.
