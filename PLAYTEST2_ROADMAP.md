...
- **NEXT (fresh context):** (1) review the Phase 4 spec-review findings → **implement Phase 4** carefully from PHASE4_EXACT_SPECS.md (4d winner-card first = lowest risk, then 4b/4c strum, then 4a CPU rival = highest risk, verify win+loss); (2) finish queued design specs (B94 kb-coachmark/best-pass, B92 #2/3/5, store-hero #9/10); (3) **Phase 6** (task #165, XL); (4) keep cycling critique rounds.
