...
- **NEXT (fresh context):** (1) apply QA round #3 regression findings (in flight — narrow re-audit of build88+89 diff); (2) **implement Phase 4** (task #163, CAREFUL — gameplay/input: CPU rival decoupled so you can LOSE, GH overstrum + chord fret-settle behind Strict-Strum toggle default OFF, keyboard byte-identical — spec in w1bxkg5rd); (3) apply the **24 design-evolution ideas** in QA2_FINDINGS.md §B (juice/onboarding/store/cohesion — pick DO-NOW); (4) **Phase 6** (task #165, XL — AI Flixs playable video-bg level [blocked on decodable audio_url] + Local Versus ?couch=1); (5) more critique/evolution rounds.
