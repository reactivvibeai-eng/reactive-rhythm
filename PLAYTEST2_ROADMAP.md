...
- 2026-06-25: **Phase 3B REMAINING** (build86): 3.3 combo 11-24 dead-zone cue (~combo 15; lightning is at %25, reveal at 10 — game.js combo milestone ~line 3040) + stronger on-highway "OVERDRIVE READY" banner (overdrive>=1 && !odActive; existing #hud-od-block ready state) + 3.4 mobile tap-zone faint outlines (.tap-zone CSS) + one-time first-countdown coachmark (touch only) + unmissable first-run How-To (tryShowHowto). Spec: workflow wbad0afjh output (items 6-8). Then **Phase 4**.
