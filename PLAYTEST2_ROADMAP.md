...
- **NEXT (fresh context):** (1) **Phase 4** (task #163 — the marquee unbuilt feature: shadow-chart CPU rival at multiplayer.js:2869 [today rival score = player×0.92 = unloseable] + GH strict-strum, KEYBOARD BYTE-IDENTICAL per PHASE4_DESIGN_PLAN.md §A — exact paste-ready specs in flight); (2) finish the queued design specs above (build96+); (3) **Phase 6** (task #165, XL); (4) keep cycling critique rounds.
